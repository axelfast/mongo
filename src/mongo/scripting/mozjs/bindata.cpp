...
void BinDataInfo::Functions::UUID::call(JSContext* cx, JS::CallArgs args) {
    boost::optional<monger::UUID> uuid;

    if (args.length() == 0) {
        uuid = monger::UUID::gen();
    } else {
        ...
        uuid = uassertStatusOK(monger::UUID::parse(str));
    };
    ConstDataRange cdr = uuid->toCDR();
    std::string encoded = monger::base64::encode(cdr.data(), cdr.length());
...
void BinDataInfo::Functions::toString::call(JSContext* cx, JS::CallArgs args) {
    ...
    if (binType == newUUID) {
        auto decoded = monger::base64::decode(*str);

        // If this is in fact a UUID, use a more friendly string representation.
        if (decoded.length() == monger::UUID::kNumBytes) {
            monger::UUID uuid = monger::UUID::fromCDR({decoded.data(), decoded.length()});
...
void BinDataInfo::Functions::hex::call(JSContext* cx, JS::CallArgs args) {
    ...
    std::string data = monger::base64::decode(*str);
    std::stringstream ss;
