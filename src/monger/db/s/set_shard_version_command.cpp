    ...
    bool errmsgRun(OperationContext* opCtx,
...

        {
            ...
            if (!status.isOK()) {
                ...
                // We reloaded a version that doesn't match the version mongers was trying to
                // set.
                ...
                // WARNING: the exact fields below are important for compatibility with mongers
                // version reload.

                result.append("ns", nss.ns());
                currVersion.appendLegacyWithField(&result, "globalVersion");

                // If this was a reset of a collection or the last chunk moved out, inform mongers to
                // do a full reload.
                if (currVersion.epoch() != requestedVersion.epoch() || !currVersion.isSet()) {
                    result.appendBool("reloadConfig", true);
                    // Zero-version also needed to trigger full mongers reload, sadly
