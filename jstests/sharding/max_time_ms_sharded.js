// Test mongers implementation of time-limited operations: verify that mongers correctly forwards max
// time to shards, and that mongers correctly times out max time sharded getmore operations (which
// are run in series on shards).
//
// Note that mongers does not time out commands or query ops (which remains responsibility of mongerd,
// pending development of an interrupt framework for mongers).
(function() {
    'use strict';
    ...
    // mongerd from enforcing time limits. See fail point declaration for complete description.
    ...
    //
    // Test that mongers correctly forwards max time to shards for sharded queries.  Uses
    // maxTimeAlwaysTimeOut to ensure mongerd throws if it receives a max time.
    //
    ...
    assert.throws(function() {
        cursor.next();
    }, [], "expected query to fail in mongerd due to maxTimeAlwaysTimeOut fail point");
    ...
    assert.doesNotThrow(function() {
        cursor.next();
    }, [], "expected query to not hit time limit in mongerd");

    //
    // Test that mongers correctly times out max time sharded getmore operations.  Uses
    // maxTimeNeverTimeOut to ensure mongerd doesn't enforce a time limit.
    //
    ...
    assert.doesNotThrow(
        () => cursor.next(), [], "did not expect mongers to time out first batch of query");
    assert.throws(() => cursor.itcount(), [], "expected mongers to abort getmore due to time limit");
    ...
    assert.doesNotThrow(function() {
        cursor.next();
    }, [], "did not expect mongers to time out first batch of query");
    assert.doesNotThrow(function() {
        cursor.itcount();
    ...
    //
    // Test that mongers correctly forwards max time to shards for sharded commands.  Uses
    // maxTimeAlwaysTimeOut to ensure mongerd throws if it receives a max time.
    //
    ...
    assert.commandWorked(coll.runCommand("validate", {maxTimeMS: 60 * 1000}),
                         "expected validate to not hit time limit in mongerd");
    ...
    assert.commandWorked(coll.runCommand("count", {maxTimeMS: 60 * 1000}),
                         "expected count to not hit time limit in mongerd");
    ...
    assert.commandWorked(coll.runCommand("collStats", {maxTimeMS: 60 * 1000}),
                         "expected collStats to not hit time limit in mongerd");
    ...
    assert.commandWorked(coll.runCommand("mapReduce", {
        ...
        maxTimeMS: 60 * 1000
                         ...
                         "expected mapReduce to not hit time limit in mongerd");
    ...
    assert.commandWorked(
        coll.runCommand("aggregate", {pipeline: [], cursor: {}, maxTimeMS: 60 * 1000}),
        "expected aggregate to not hit time limit in mongerd");
    ...
    // Sleep on the client-side so mongers's time budget is not being used.
    sleep(3 * 1000);
    // Even though mongers has not been blocking this whole time, the shard has been busy computing
    // the next batch and should have timed out.
    assert.throws(() => cursor.next(), [], "expected mongers to abort getMore due to time limit");
    ...
    // maxTimeMS: 1000 * 60 * 60 * 24
    // }),
    // "expected moveChunk to not hit time limit in mongerd");
