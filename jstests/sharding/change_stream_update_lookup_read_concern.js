...
(function() {
    ...
    // Shard the collection to ensure the change stream will perform update lookup from mongers.
    ...
    const closestSecondaryDB = closestSecondary.getDB(mongersDB.getName());
    ...
    profilerHasSingleMatchingEntryOrThrow({
        ...
        filter: {ns: mongersColl.getFullName(), "command.comment": "testing targeting"}
    });
    ...
    assert.writeOK(mongersColl.update({_id: 1}, {$set: {updatedCount: 1}}));
    ...
    profilerHasSingleMatchingEntryOrThrow({
        ...
        errorMsgFilter: {ns: mongersColl.getFullName()},
    ...
    const newClosestSecondaryDB = newClosestSecondary.getDB(mongersDB.getName());
    const originalClosestSecondaryDB = closestSecondaryDB;

    // Wait for the mongers to acknowledge the new tags from our reconfig.
    ...
    assert.writeOK(mongersColl.update({_id: 1}, {$set: {updatedCount: 2}}));
    ...
    profilerHasSingleMatchingEntryOrThrow({
        ...
        filter: {
            op: "query",
            ns: mongersColl.getFullName(), "command.comment": changeStreamComment,
            // We need to filter out any profiler entries with a stale config - this is the first
            // read on this secondary with a readConcern specified, so it is the first read on this
            // secondary that will enforce shard version.
            errCode: {$ne: ErrorCodes.StaleConfig}
