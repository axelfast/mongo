 ...
 * query/write metrics where applicable, on both mongerD and mongerS and under both legacy and command
 * protocols.
...
(function() {
    ...
    const mongersDB = mongersConn.getDB(dbName);
    ...
    st.ensurePrimaryShard(mongersDB.getName(), shardConn.name);
    ...
    function dropAndRecreateTestCollection() {
        assert(mongersDB.test.drop());
        st.shardColl(mongersDB.test, {_id: 1}, {_id: 0}, {_id: 1}, mongersDB.getName(), true);
    }
    ...
    function runLoggingTests({db, readWriteMode, slowMs, logLevel, sampleRate}) {
        ...
        function assertWriteOK(writeResponse) {
            if (!writeResponse) {
                assert(db.getMonger().writeMode !== "commands");
        ...
        db.getMonger().forceWriteMode(readWriteMode);
        db.getMonger().forceReadMode(readWriteMode);
        ...
        const logFormatTestComment = (isMongers ? 'mongers' : 'mongerd') + "_" + readWriteMode +
        ...
        // Certain fields in the log lines on mongerD are not applicable in their counterparts on
        // mongerS, and vice-versa. Ignore these fields when examining the logs of an instance on
        ...
        const testList = [
            ...
            },
            {
              ...
              // TODO SERVER-34208: display FAM update metrics in mongerS logs.
              logFields: Object.assign((isMongers ? {} : {nMatched: 1, nModified: 1}), {
    ...
    for (let testDB of[shardDB, mongersDB]) {
