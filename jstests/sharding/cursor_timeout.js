...
// mongerd and mongers.  This test creates the following four cursors:
//
// 1. A no-timeout cursor through mongers.
// 2. A no-timeout cursor through mongerd.
// 3. A normal cursor through mongers.
// 4. A normal cursor through mongerd.
//
...
(function() {
    ...
    // Cursor timeout on mongerd is handled by a single thread/timer that will sleep for
    ...
    // of 4 to 5 seconds.
    const mongerdCursorTimeoutMs = 5000;

    // Cursor timeout on mongers is handled by checking whether the "last accessed" cursor time stamp
    ...
    const mongersCursorTimeoutMs = 4000;
    ...
    const st = new ShardingTest({
        ...
        other: {
            shardOptions: {
                verbose: 1,
                setParameter: {
                    cursorTimeoutMillis: mongerdCursorTimeoutMs,
            ...
            mongersOptions: {
                verbose: 1,
                setParameter: {
                    cursorTimeoutMillis: mongersCursorTimeoutMs,
                    clientCursorMonitorFrequencySecs: cursorMonitorFrequencySecs
                }
            },
        },
    ...
    // Open both a normal and a no-timeout cursor on mongers. Batch size is 1 to ensure that
    ...
    // Open both a normal and a no-timeout cursor on mongerd. Batch size is 1 to ensure that
    // cursor.next() performs only a single operation.
    ...
    // routerCursorWithTimeout is killed by a killCursors command from the mongers.
    assert.soon(function() {
