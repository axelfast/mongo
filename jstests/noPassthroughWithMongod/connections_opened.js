...
// We need to create a new mongerd to ensure no one else is talking to us in the background, and
// will screw up our stats.
var monger = MongerRunner.runMongerd({});
var db = monger.getDB("test");
...
function createPersistentConnection() {
    assert.soon(function() {
        try {
            permConns.push(new Monger(db.getMonger().host));
            return true;
        } catch (x) {
            return false;
        }
...
function createTemporaryConnection() {
    ...
    var pollString = "var conn = null;" + "assert.soon(function() {" + "try { conn = new Monger(\"" +
        db.getMonger().host + "\"); return conn" + "} catch (x) {return false;}}, " +
