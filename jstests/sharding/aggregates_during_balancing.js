...
(function() {
    ...
    var shardedAggTest =
        new ShardingTest({shards: 2, mongers: 1, other: {chunkSize: 1, enableBalancer: true}});
    ...
    /*
    Test combining results in mongers for operations that sub-aggregate on shards.
    ...
    so that these can be combined in mongers by totalling the subtotals counts
    before performing the final division.  For $pushToSet and $push, the shard
    pipelines produce arrays, but in mongers these are combined rather than simply
    ...
    */
    ...
    jsTestLog('a project and group in shards, result combined in mongers');
    var a1 = db.ts1
                 .aggregate([
    ...
    jsTestLog('an initial group starts the group in the shards, and combines them in mongers');
    ...
    jsTestLog('a match takes place in the shards; just returning the results from mongers');
    var a4 = db.ts1
                 .aggregate([{
                     $match: {
                         $or: [
    ...
    function testSkipLimit(ops, expectedCount) {
        ...
        if (expectedCount > 10) {
            // make shard -> mongers intermediate results less than 16MB
