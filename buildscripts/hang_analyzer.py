...
def get_process_logger(debugger_output, pid, process_name):
    ...
    process_logger.monger_process_filename = None
    ...
    if 'file' in debugger_output:
        ...
        process_logger.monger_process_filename = filename
...
class GDBDumper(object):
    ...
    def __find_debugger(debugger):
        """Find the installed debugger."""
        return find_program(debugger, ['/opt/mongerdbtoolchain/gdb/bin', '/usr/bin'])
    ...
    def dump_info(  # pylint: disable=too-many-arguments,too-many-locals
        ...
        monger_script = os.path.join(gdb_dir, "monger.py")
        monger_printers_script = os.path.join(gdb_dir, "monger_printers.py")
        monger_lock_script = os.path.join(gdb_dir, "monger_lock.py")

        source_monger = "source %s" % monger_script
        source_monger_printers = "source %s" % monger_printers_script
        source_monger_lock = "source %s" % monger_lock_script
        mongerdb_dump_locks = "mongerdb-dump-locks"
        mongerdb_show_locks = "mongerdb-show-locks"
        mongerdb_uniqstack = "mongerdb-uniqstack mongerdb-bt-if-active"
        mongerdb_waitsfor_graph = "mongerdb-waitsfor-graph debugger_waitsfor_%s_%d.gv" % \
            (process_name, pid)
        ...
        if sys.platform.startswith("sunos"):
            ...
            mongerdb_show_locks = ""
            mongerdb_waitsfor_graph = ""
            mongerdb_javascript_stack = ""
            mongerd_dump_sessions = ""

        if not logger.monger_process_filename:
            raw_stacks_commands = []
        else:
            base, ext = os.path.splitext(logger.monger_process_filename)
...
def main():  # pylint: disable=too-many-branches,too-many-locals,too-many-statements
    ...
    interesting_processes = ["monger", "mongerd", "mongers", "_test", "dbtest", "python", "java"]
    go_processes = []
    process_ids = []
