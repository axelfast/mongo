...
(function() {
    ...
    assert.commandWorked(mongers.adminCommand({addShardToZone: shardName, zone: 'x'}));
    var shardDoc = config.shards.findOne();
    assert.eq(['x'], shardDoc.tags);

    // Test adding zone to a shard with existing zones.
    assert.commandWorked(mongers.adminCommand({addShardToZone: shardName, zone: 'y'}));
    shardDoc = config.shards.findOne();
    assert.eq(['x', 'y'], shardDoc.tags);

    // Test removing shard from existing zone.
    assert.commandWorked(mongers.adminCommand({removeShardFromZone: shardName, zone: 'x'}));
    shardDoc = config.shards.findOne();
    ...
    assert.commandWorked(mongers.adminCommand({removeShardFromZone: shardName, zone: 'y'}));
    shardDoc = config.shards.findOne();
