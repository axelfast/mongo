...
(function() {
     ...
     */
    var connPoolStats = mongers.getDB('admin').runCommand({connPoolStats: 1});
    ...
    awaitRSClientHosts(mongers, {host: targetHostName}, {ok: true, ismaster: true});
    ...
    jsTest.log("Waiting for mongers to reflect change in shard replica set membership.");
    var replView;
    assert.soon(
        function() {
            var connPoolStats = mongers.getDB('admin').runCommand('connPoolStats');
    ...
    assert.soon(
        function() {
            shardDoc = mongers.getDB('config').shards.findOne();
