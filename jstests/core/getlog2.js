...
(function() {
    ...
    // We turn off gossiping the monger shell's clusterTime because it causes the slow command log
    ...
    // test doesn't work when talking to mongers
    ...
    assert(contains(query.log, function(v) {
        print(v);
        const opString = db.getMonger().useReadCommands() ? " find " : " query ";
        const filterString = db.getMonger().useReadCommands() ? "filter:" : "command:";
