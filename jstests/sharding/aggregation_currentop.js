 ...
 * - Specifying {localOps: true} shows the local ops on mongerS rather than shard ops.
...
(function() {
    ...
    // temporarily set yields to hang the mongerd so we can capture particular operations in the
    // currentOp output.
    ...
    let mongersConn = st.s;
    let shardRS = st.rs0;

    let clusterTestDB = mongersConn.getDB(jsTestName());
    let clusterAdminDB = mongersConn.getDB("admin");
    ...
    function restartCluster(st, newOpts) {
        ...
        mongersConn = st.s;
        shardRS = st.rs0;
        clusterTestDB = mongersConn.getDB(jsTestName());
        clusterAdminDB = mongersConn.getDB("admin");
    ...
    // Run the common tests on a shard, through mongerS, and on mongerS with 'localOps' enabled.
    runCommonTests(shardConn);
    runCommonTests(mongersConn);
    runCommonTests(mongersConn, {localOps: true});

    //
    // mongerS specific tests.
    //

    // Test that a user without the inprog privilege cannot run non-local $currentOp via mongerS even
    ...
    // mongerS even if $ownOps is true.
    assert.commandFailedWithCode(clusterAdminDB.currentOp({$ownOps: true}),
                                 ErrorCodes.Unauthorized);

    // Test that a non-local $currentOp pipeline via mongerS returns results from all shards, and
    ...
    // Test that a $currentOp pipeline with {localOps:true} returns operations from the mongerS
    ...
    assert.eq(clusterAdminDB
                  .aggregate(
                      [
                        ...
                        {
                          $match: {
                              $expr: {$eq: ["$host", "$clientMetadata.mongers.host"]},
                              "command.comment": "mongers_currentop_localOps"
                          }
                        }
                      ],
                      {comment: "mongers_currentop_localOps"})
                  .itcount(),
              1);
    ...
    // Runs a suite of tests for behaviour common to both replica sets and mongerS with
    // {localOps:true}.
    ...
    // Run the localOps tests for both replset and mongerS.
    runLocalOpsTests(mongersConn);
    ...
    // Run a set of tests of behaviour common to replset and mongerS when auth is disabled.
    ...
    runNoAuthTests(mongersConn);
    runNoAuthTests(mongersConn, {localOps: true});
    ...
    // Test that the host field is present and the shard field is absent when run on mongerD.
    assert.eq(shardAdminDB
                  .aggregate([
                      {$currentOp: {allUsers: true, idleConnections: true}},
                      {$group: {_id: {shard: "$shard", host: "$host"}}}
    ...
    // Test that $currentOp can run while the mongerD is write-locked.
