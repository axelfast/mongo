(function() {
    ...
    var checkShellOkay = function(url) {
        ...
        var argv = ['./monger', url, '--eval', ('db.runCommand({replSetGetStatus: 1})')];
        if (!_isWindows()) {
    ...
    var noMentionSSLURL = `mongerdb://${nodeList}/admin?replicaSet=${replTest.name}`;
    ...
    var useSSLURL = `mongerdb://${nodeList}/admin?replicaSet=${replTest.name}&ssl=true`;
    jsTestLog(`Replica set url (uses SSL): ${useSSLURL}`);
    assert.eq(checkShellOkay(useSSLURL), 0, "successfully connected with SSL");

    var disableSSLURL = `mongerdb://${nodeList}/admin?replicaSet=${replTest.name}&ssl=false`;
