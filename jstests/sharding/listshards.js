...
(function() {
    ...
    var shardTest = new ShardingTest(
        {name: 'listShardsTest', shards: 1, mongers: 1, other: {useHostname: true}});

    var mongers = shardTest.s0;
    var res = mongers.adminCommand('listShards');
    assert.commandWorked(res, 'listShards command failed');
    var shardsArray = res.shards;
    assert.eq(shardsArray.length, 1);

    // add standalone mongerd
    var standaloneShard = MongerRunner.runMongerd({useHostName: true, shardsvr: ""});
    ...
    res = mongers.adminCommand('listShards');
    assert.commandWorked(res, 'listShards command failed');
    ...
    rs1.stopSet();
    shardTest.stop();
    MongerRunner.stopMongerd(standaloneShard);
