/**
 * Mongers has special targeting behavior for createIndex, reIndex, dropIndex, and collMod:
 *
...
(function() {
    ...
    assert.eq(res.code, res.raw[st.shard0.host].code, tojson(res));
    assert.eq(res.codeName, res.raw[st.shard0.host].codeName, tojson(res));
    // We might see 'HostUnreachable' the first time if the mongers's ReplicaSetMonitor does not yet
    ...
    // createIndexes attempt, mongers's ReplicaSetMonitor should have been updated.
