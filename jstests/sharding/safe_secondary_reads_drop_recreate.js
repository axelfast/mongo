...
(function() {
    ...
    let testCases = {
        ...
        aggregate: {
            setUp: function(mongersConn) {
                assert.writeOK(mongersConn.getCollection(nss).insert({x: 1}));
            },
        ...
        geoSearch: {skip: "not supported in mongers"},
    ...
    let scenarios = {
        dropRecreateAsUnshardedOnSameShard: function(
            staleMongers, freshMongers, test, commandProfile) {
            let primaryShardPrimary = st.rs0.getPrimary();
            let primaryShardSecondary = st.rs0.getSecondary();

            // Drop and recreate the collection.
            assert.commandWorked(freshMongers.getDB(db).runCommand({drop: coll}));
            assert.commandWorked(freshMongers.getDB(db).runCommand({create: coll}));
            ...
            let res = staleMongers.getDB(db).runCommand(Object.assign(
                {},
                test.command,
        ...
        dropRecreateAsShardedOnSameShard: function(staleMongers, freshMongers, test, commandProfile) {
            let primaryShardPrimary = st.rs0.getPrimary();
            let primaryShardSecondary = st.rs0.getSecondary();
            ...
            assert.commandWorked(freshMongers.getDB(db).runCommand({drop: coll}));
            assert.commandWorked(freshMongers.getDB(db).runCommand({create: coll}));
            assert.commandWorked(freshMongers.adminCommand({shardCollection: nss, key: {x: 1}}));

            // We do this because we expect staleMongers to see that the collection is sharded, which
        ...
        dropRecreateAsUnshardedOnDifferentShard: function(
            ...
            // shard without calling movePrimary, and it is known that a stale mongers will not
    ...
    let st = new ShardingTest({mongers: 2, shards: {rs0: rsOpts, rs1: rsOpts}});
    ...
    for (let command of commands) {
        ...
        for (let scenario in scenarios) {
            ...
            assert.commandWorked(staleMongers.adminCommand({enableSharding: db}));
            st.ensurePrimaryShard(db, st.shard0.shardName);
            assert.commandWorked(staleMongers.adminCommand({shardCollection: nss, key: {x: 1}}));
            ...
            // Do dummy read from the stale mongers so it loads the routing table into memory once.
            // Additionally, do a secondary read to ensure that the secondary has loaded the initial
            // routing table -- the first read to the primary will refresh the mongers' shardVersion,
            // which will then be used against the secondary to ensure the secondary is fresh.
            assert.commandWorked(staleMongers.getDB(db).runCommand({find: coll}));
            assert.commandWorked(freshMongers.getDB(db).runCommand({
                find: coll,
            ...
            scenarios[scenario](staleMongers, freshMongers, test, commandProfile);

            // Clean up the database by dropping it; this is the only way to drop the profiler
            // collection on secondaries.
            // Do this from staleMongers, so staleMongers purges the database entry from its cache.
            assert.commandWorked(staleMongers.getDB(db).runCommand({dropDatabase: 1}));
        }
    }
