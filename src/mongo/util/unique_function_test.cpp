...
TEST(UniqueFunctionTest, construct_simple_unique_function_from_lambda) {
    // Implicit construction
    {
        ...
        monger::unique_function<void()> uf = [] { RunDetection<0>::itRan = true; };
...
TEST(UniqueFunctionTest, assign_simple_unique_function_from_lambda) {
    // Implicit construction
    RunDetection<0> runDetection;
    monger::unique_function<void()> uf;
...
TEST(UniqueFunctionTest, accepts_a_functor_that_is_move_only) {
    ...
    monger::unique_function<void()> uf = [checkerPtr = std::make_unique<Checker>()]{};

    monger::unique_function<void()> uf2 = std::move(uf);
...
TEST(UniqueFunctionTest, simple_instantiations) {
    monger::unique_function<void()> a;

    monger::unique_function<void()> x = []() -> int { return 42; };
    x = []() -> int { return 42; };
}

namespace conversion_checking {
template <typename FT>
using uf = monger::unique_function<FT>;
template <typename FT>
...
#ifndef _MSC_VER
    // Note that `monger::unique_function` must never convert to `std::function` in any of the
    // following cases.
