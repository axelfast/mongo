// Test --host.
(function() {
    // This "inner_mode" method of spawning a mongerd and re-running was copied from
    ...
    if ("undefined" == typeof inner_mode) {
        ...
        // with that address.
        var mongerd = MongerRunner.runMongerd({ipv6: "", bind_ip: "::1,127.0.0.1"});
        if (mongerd == null) {
        ...
        var args = [
            ...
            "jstests/noPassthroughWithMongerd/host_connection_string_validation.js"
        ];
        var exitCode = _runMongerProgram.apply(null, args);
    ...
    var testHost = function(host, shouldSucceed) {
        var exitCode = runMongerProgram('monger', '--ipv6', '--eval', ';', '--host', host);
        if (shouldSucceed) {
            if (exitCode !== 0) {
    ...
    function runUriTestFor(i, connectionString, isGood) {
        ...
        print("    * testing mongerdb://" + encodeURIComponent(connectionString));
        testHost("mongerdb://" + encodeURIComponent(connectionString), isGood);
    }
