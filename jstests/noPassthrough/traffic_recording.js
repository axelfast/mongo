...
(function() {
    ...
    function runTest(client, restartCommand) {
        ...
        const path = MongerRunner.toRealDir("$dataDir/traffic_recording/");
        mkdir(path);

        if (!jsTest.isMongers(client)) {
    ...
    {
        let m = MongerRunner.runMongerd({auth: ""});
        ...
        m = runTest(m, function(setParams) {
            if (m) {
                MongerRunner.stopMongerd(m, null, {user: 'admin', pwd: 'pass'});
            }
            m = MongerRunner.runMongerd({auth: "", setParameter: setParams});
