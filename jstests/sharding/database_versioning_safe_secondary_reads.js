...
(function() {
    ...
    // Make "staleMongers" load the stale database info into memory.
    const freshMongers = st.s0;
    const staleMongers = st.s1;
    staleMongers.getDB(dbName).runCommand({listCollections: 1});
    ...
    assert.commandWorked(freshMongers.adminCommand({movePrimary: dbName, to: st.shard1.shardName}));
    const dbEntry2 = freshMongers.getDB("config").getCollection("databases").findOne({_id: dbName});
    ...
    // Run listCollections with readPref=secondary from the stale mongers. First, this should cause
    // the old primary shard's secondary to provoke the old primary shard's primary to refresh. Then
    // once the stale mongers refreshes, it should cause the new primary shard's secondary to provoke
    ...
    assert.commandWorked(staleMongers.getDB(dbName).runCommand(
        {listCollections: 1, $readPreference: {mode: "secondary"}, readConcern: {level: "local"}}));
    ...
    assert.commandWorked(freshMongers.getDB(dbName).runCommand({dropDatabase: 1}));
    ...
    // different shard, a stale mongers that has cached the old database's primary shard will *not*
