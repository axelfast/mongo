 ...
 *      2. It must be running with mongerbridge.
 *
...
function RollbackTestDeluxe(name = "FiveNodeDoubleRollbackTest", replSet) {
    ...
    function validateAndUseSetup(replSet) {
        ...
        assert.eq(true, replSet.usesBridge(), "Must set up ReplSetTest with mongerbridge enabled.");
    ...
    this.restartNode = function(nodeId, signal) {
        ...
        let opts = {};
        if (signal === SIGKILL) {
            opts = {allowedExitCode: MongerRunner.EXIT_SIGKILL};
        }
