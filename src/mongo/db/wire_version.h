 ...
 * mandatory.
 *
 * We use the wire version to determine if two agents (a driver, a mongers, or a mongerd) can
...
enum WireVersion {
    ...
    // Support find and getMore commands, as well as OP_COMMAND in mongerd (but not mongers).
    FIND_COMMAND = 4,
...
struct WireSpec {
    ...
    // [COMMANDS_ACCEPT_WRITE_CONCERN, SUPPORTS_OP_MSG] and it is a mongerd, the initiating node will
    ...
    // version range [COMMANDS_ACCEPT_WRITE_CONCERN, SUPPORTS_OP_MSG] and it is a mongerd, the
    ...
    // Set to true if the client is internal to the cluster---this is a mongerd or mongers connecting
    // to another mongerd.
