...
(function() {
    ...
    const mongerd = MongerRunner.runMongerd(x509_options);

    // Use new toolchain python, if it exists
    let python_binary = '/opt/mongerdbtoolchain/v3/bin/python3';
    if (runProgram('/bin/sh', '-c', 'ls ' + python_binary) !== 0) {
        python_binary = '/opt/mongerdbtoolchain/v3/bin/python3';
    }

    // Run the tls cipher suite enumerator
    const python = '/usr/bin/env ' + python_binary;
    const enumerator = " jstests/ssl/tls_enumerator.py ";
    const python_command = python + enumerator + '--port=' + mongerd.port + ' --cafile=' + CA_CERT +
