...
const (
	...
	FlagDiscover             // only active when mongerstat is in discover mode
	...
	FlagAll                  // only active if mongerstat was run with --all option
	FlagMMAP                 // only active if node has mmap-specific fields
	FlagWT                   // only active if node has wiredtiger-specific fields
)

// StatHeader describes a single column for mongerstat's terminal output,
...
// StatHeaders are the complete set of data metrics supported by mongerstat.
var (
