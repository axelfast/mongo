...
(function() {
    ...
    function assertBinVersionsEqual(v1, v2) {
        assert(MongerRunner.areBinVersionsTheSame(v1, v2),
    ...
    function assertBinVersionComparesHigher(v1, v2) {
        assert.eq(1,
                  MongerRunner.compareBinVersions(v1, v2),
    ...
    assert.throws(MongerRunner.areBinVersionsTheSame, ["3", "3.2"]);
    assert.throws(MongerRunner.areBinVersionsTheSame, ["3.2", "3"]);

    // Throw an error when versions differ only by githash.
    assert.throws(MongerRunner.compareBinVersions, ["3.4.1-abc", "3.4.1-xyz"]);
}());
