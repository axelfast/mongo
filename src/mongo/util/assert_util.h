...
#include "monger/base/status.h"  // NOTE: This is safe as utils depend on base
...
/** Most monger exceptions inherit from this; this is commonly caught in most threads */
...
#define MONGO_fassertFailed(...) ::monger::fassertFailedWithLocation(__VA_ARGS__, __FILE__, __LINE__)
...
#define MONGO_fassertFailedNoTrace(...) \
    ::monger::fassertFailedNoTraceWithLocation(__VA_ARGS__, __FILE__, __LINE__)
...
#define MONGO_fassertFailedWithStatus(...) \
    ::monger::fassertFailedWithStatusWithLocation(__VA_ARGS__, __FILE__, __LINE__)
...
#define MONGO_fassertFailedWithStatusNoTrace(...) \
    ::monger::fassertFailedWithStatusNoTraceWithLocation(__VA_ARGS__, __FILE__, __LINE__)
...
#define MONGO_fassert(...) ::monger::fassertWithLocation(__VA_ARGS__, __FILE__, __LINE__)
...
#define MONGO_fassertNoTrace(...) \
    ::monger::fassertNoTraceWithLocation(__VA_ARGS__, __FILE__, __LINE__)
...
#define MONGO_BASE_ASSERT_FAILED(fail_func, code, msg)                                    \
    do {                                                                                  \
        [&]() MONGO_COMPILER_COLD_FUNCTION {                                              \
            fail_func(::monger::error_details::makeStatus(code, msg), __FILE__, __LINE__); \
        }();                                                                              \
 ...
 */
#define uasserted(msgid, msg) MONGO_BASE_ASSERT_FAILED(::monger::uassertedWithLocation, msgid, msg)
#define uassert(msgid, msg, expr) \
    MONGO_BASE_ASSERT(::monger::uassertedWithLocation, msgid, msg, expr)

MONGO_COMPILER_NORETURN void uassertedWithLocation(const Status& status,
                                                   const char* file,
                                                   unsigned line);

#define uassertStatusOK(...) ::monger::uassertStatusOKWithLocation(__VA_ARGS__, __FILE__, __LINE__)
...
#define uassertStatusOKWithContext(status, contextExpr) \
    ::monger::uassertStatusOKWithContextAndLocation(     \
...
#define massert(msgid, msg, expr) \
    MONGO_BASE_ASSERT(::monger::msgassertedWithLocation, msgid, msg, expr)

#define msgasserted(msgid, msg) \
    MONGO_BASE_ASSERT_FAILED(::monger::msgassertedWithLocation, msgid, msg)
MONGO_COMPILER_NORETURN void msgassertedWithLocation(const Status& status,
                                                     const char* file,
                                                     unsigned line);

#define massertStatusOK(...) ::monger::massertStatusOKWithLocation(__VA_ARGS__, __FILE__, __LINE__)
...
#define MONGO_verify(_Expression)                                    \
    do {                                                             \
        if (MONGO_unlikely(!(_Expression))) {                        \
            ::monger::verifyFailed(#_Expression, __FILE__, __LINE__); \
        }                                                            \
    } while (false)
...
inline void invariantWithLocation(const Status& status,
                                  ...
                                  unsigned line) {
    if (MONGO_unlikely(!status.isOK())) {
        ::monger::invariantOKFailed(expr, status, file, line);
    }
}
...
inline T invariantWithLocation(StatusWith<T> sw,
                               ...
                               unsigned line) {
    if (MONGO_unlikely(!sw.isOK())) {
        ::monger::invariantOKFailed(expr, sw.getStatus(), file, line);
    }
    return std::move(sw.getValue());
}
...
inline void invariantWithContextAndLocation(const Status& status,
                                            ...
                                            unsigned line) {
    if (MONGO_unlikely(!status.isOK())) {
        ::monger::invariantOKFailedWithMsg(
...
inline T invariantWithContextAndLocation(StatusWith<T> sw,
                                         ...
                                         unsigned line) {
    if (MONGO_unlikely(!sw.isOK())) {
        ::monger::invariantOKFailedWithMsg(expr, sw.getStatus(), contextExpr(), file, line);
    }
    return std::move(sw.getValue());
}
...
#define MONGO_UNREACHABLE ::monger::invariantFailed("Hit a MONGO_UNREACHABLE!", __FILE__, __LINE__);
