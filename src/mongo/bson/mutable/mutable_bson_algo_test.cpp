...
using monger::CollatorInterfaceMock;
using monger::Status;
using namespace monger::mutablebson;

class DocumentTest : public monger::unittest::Test {
public:
    DocumentTest() : _doc() {}
...
TEST(WoLessTest, CollationAware) {
    ...
    Document less(monger::fromjson("{ x: 'cbc' }"));
    Document greater(monger::fromjson("{ x: 'abd' }"));
...
TEST(WoEqualTest, CollationAware) {
    ...
    Document docA(monger::fromjson("{ x: 'not' }"));
    Document docB(monger::fromjson("{ x: 'equal' }"));
