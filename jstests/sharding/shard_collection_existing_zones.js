...
(function() {
    ...
    var st = new ShardingTest({mongers: 1, shards: 3});
    var kDbName = 'test';
    var kCollName = 'foo';
    var ns = kDbName + '.' + kCollName;
    var zoneName = 'zoneName';
    var mongers = st.s0;
    var testDB = mongers.getDB(kDbName);
    ...
    function testShardZoneAssociationValidation(proposedShardKey, numberLongMin, numberLongMax) {
        ...
        assert.commandFailed(mongers.adminCommand({shardCollection: ns, key: proposedShardKey}));

        assert.commandWorked(st.s.adminCommand({addShardToZone: shardName, zone: zoneName}));
        assert.commandWorked(mongers.adminCommand({shardCollection: ns, key: proposedShardKey}));
    ...
    function testChunkSplits(collectionExists) {
        ...
        assert.commandWorked(mongers.adminCommand({shardCollection: ns, key: shardKey}));
        var expectedChunks = [
    ...
    function testNonemptyZonedCollection() {
        ...
        for (let i = 0; i < shards.length; i++) {
            assert.commandWorked(
                mongers.adminCommand({addShardToZone: shards[i]._id, zone: zoneName + i}));
