...
def get_all_source_files(arr=None, prefix="."):
    ...
    for fx in os.listdir(prefix):
        ...
        if (fx.startswith(".") or fx.startswith("pcre-") or fx.startswith("32bit")
                or fx.startswith("mongerdb-") or fx.startswith("debian")
                or fx.startswith("monger-cxx-driver") or fx.startswith("sqlite") or "gotools" in fx
                or fx.find("mozjs") != -1):
            continue
...
def get_git_branch_string(prefix="", postfix=""):
    ...
    if len(tt) > 2 and tt[len(tt) - 1] == "monger":
        par = tt[len(tt) - 2]
