...
func (socket *MongoSocket) ServerInfo() *mongerServerInfo {
	socket.Lock()
...
func (socket *MongoSocket) InitialAcquire(serverInfo *mongerServerInfo, timeout time.Duration) error {
	socket.Lock()
...
func (socket *MongoSocket) Acquire() (info *mongerServerInfo) {
	socket.Lock()
	if socket.references == 0 {
