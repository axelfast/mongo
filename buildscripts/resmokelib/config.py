...
# Subdirectory under the dbpath prefix that contains directories with data files of mongerd's started
# by resmoke.py.
FIXTURE_SUBDIR = "resmoke"

# Subdirectory under the dbpath prefix that contains directories with data files of mongerd's started
# by individual tests.
MONGO_RUNNER_SUBDIR = "mongerrunner"
...
DEFAULT_MONGO_EXECUTABLE = os.path.join(os.curdir, "monger")
DEFAULT_MONGOEBENCH_EXECUTABLE = os.path.join(os.curdir, "mongerebench")
DEFAULT_MONGOD_EXECUTABLE = os.path.join(os.curdir, "mongerd")
DEFAULT_MONGOS_EXECUTABLE = os.path.join(os.curdir, "mongers")

DEFAULT_BENCHMARK_REPETITIONS = 3
DEFAULT_BENCHMARK_MIN_TIME = datetime.timedelta(seconds=5)

# Default root directory for where resmoke.py puts directories containing data files of mongerd's it
...
DEFAULTS = {
    ...
    "buildlogger_url": "https://logkeeper.mongerdb.org",
    ...
    "jobs": 1,
    "monger_executable": None,
    "mongerd_executable": None,
    "mongerd_set_parameters": None,
    "mongerebench_executable": None,
    "mongers_executable": None,
    "mongers_set_parameters": None,
    "no_journal": False,
    ...
    "git_revision": None,
    "patch_build": False,
    "project_name": "mongerdb-monger-master",
...
# The starting port number to use for mongerd and mongers processes spawned by resmoke.py and the
# monger shell.
BASE_PORT = None
...
# Root directory for where resmoke.py puts directories containing data files of mongerd's it starts,
...
# The path to the monger executable used by resmoke.py.
MONGO_EXECUTABLE = None

# The path to the mongerd executable used by resmoke.py.
MONGOD_EXECUTABLE = None

# The --setParameter options passed to mongerd.
MONGOD_SET_PARAMETERS = None

# The path to the mongerebench executable used by resmoke.py.
MONGOEBENCH_EXECUTABLE = None

# The path to the mongers executable used by resmoke.py.
MONGOS_EXECUTABLE = None

# The --setParameter options passed to mongers.
MONGOS_SET_PARAMETERS = None

# If true, then all mongerd's started by resmoke.py and by the monger shell will not have journaling
# enabled.
NO_JOURNAL = None
...
# IF set, then mongerd/mongers's started by resmoke.py will use the specified service executor
SERVICE_EXECUTOR = None
...
# If set, then monger shells started by resmoke.py will use the specified read mode.
SHELL_READ_MODE = None

# If set, then monger shells started by resmoke.py will use the specified write mode.
SHELL_WRITE_MODE = None
...
# If set, then all mongerd's started by resmoke.py and by the monger shell will use the specified
# storage engine.
STORAGE_ENGINE = None
...
# If set, then mongerd/mongers's started by resmoke.py will use the specified transport layer.
TRANSPORT_LAYER = None
...
ARCHIVE_BUCKET = "mongerdatafiles"
