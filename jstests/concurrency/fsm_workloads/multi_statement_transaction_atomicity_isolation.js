...
var $config = (function() {
    ...
    function checkNumUpdatedByEachTransaction(documents) {
        ...
        for (let doc of documents) {
            for (let op of doc.order) {
                // We store 'op' both as the key and as part of the value because the monger shell's
    ...
    const states = (function() {
        ...
        return {
            init: function init(db, collName) {
                ...
                this.session = db.getMonger().startSession({causalConsistency: true});
    ...
    function setup(db, collName, cluster) {
        ...
        if (cluster.isSharded()) {
            ...
            cluster.synchronizeMongersClusterTimes();
        }
    }
