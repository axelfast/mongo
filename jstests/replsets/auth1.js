...
(function() {
    ...
    print("try starting mongerd with auth");
    var m = MongerRunner.runMongerd(
        {auth: "", port: port[4], dbpath: MongerRunner.dataDir + "/wrong-auth"});
    ...
    print("try starting mongerd");
    m = runMongerProgram("mongerd",
                        "--keyFile",
                        key1_644,
                        "--port",
                        port[0],
                        "--dbpath",
                        MongerRunner.dataPath + name);

    print("should fail with wrong permissions");
    assert.eq(
        m, _isWindows() ? 100 : 1, "mongerd should exit w/ 1 (EXIT_FAILURE): permissions too open");
    ...
    m = MongerRunner.runMongerd({dbpath: MongerRunner.dataPath + name + "-0"});
    ...
    var conn = MongerRunner.runMongerd({
        dbpath: MongerRunner.dataPath + name + "-3",
        port: port[3],
        replSet: "rs_auth1",
        oplogSize: 2,
        keyFile: key2_600
