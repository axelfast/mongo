...
(function() {
    ...
    const coffeeColl = mongers.getDB(dbName).getCollection("coffee");
    assert.commandWorked(mongers.adminCommand({enableSharding: dbName}));
    st.ensurePrimaryShard(dbName, shard0.shardName);
    assert.commandWorked(
        mongers.adminCommand({shardCollection: coffeeColl.getFullName(), key: {price: 1}}));
    const splitPoint = 50.0;
    assert.commandWorked(
        mongers.adminCommand({split: coffeeColl.getFullName(), middle: {price: splitPoint}}));
    ...
    const teaColl = mongers.getDB(dbName).getCollection("tea");
    const teas = [
    ...
    function assertShardsHaveExpectedData() {
        ...
        // mongers, or consulting the primary shard directly.
        assert.eq(teaColl.find().sort({_id: 1}).readConcern("majority").toArray(),
                  teas.sort(sortOn("_id")),
                  "couldn't find all unsharded data via mongers");
        ...
        assert.eq(coffeeColl.find().sort({_id: 1}).readConcern("majority").toArray(),
                  ...
                  "couldn't find all sharded data via mongers scatter-gather");
        ...
        assert.eq(coffeeColl.find({price: {$gt: 0}}).sort({price: 1}).toArray(),
                  ...
                  "couldn't find all sharded data via mongers multi-shard targeted query");

        // Assert that we can find all sharded documents on shard0 by shard targeting via mongers,
        ...
        assert.eq(coffeeColl.find({price: {$lt: splitPoint}}).sort({_id: 1}).toArray(),
                  dataShard0,
                  "couldn't find shard0 data via targeting through mongers");
        ...
        // Assert that we can find all sharded documents on shard1 by shard targeting via mongers,
        ...
        assert.eq(coffeeColl.find({price: {$gte: splitPoint}}).sort({_id: 1}).toArray(),
                  dataShard1,
                  "couldn't find shard1 data via targeting through mongers");
    ...
    function assertConfigServersHaveExpectedData() {
        const configDBViaMongers = mongers.getDB("config");
        const configDBViaConfigSvr = st.config0.getDB("config");
        const projectOnlyShard = {_id: 0, shard: 1};

        // Assert that we can find documents for chunk metadata, both via mongers and by asking the
        ...
        assert.eq(configDBViaMongers.chunks.find(smallestChunk, projectOnlyShard).toArray(),
                  [smallestChunkShard]);
        ...
        assert.eq(configDBViaMongers.chunks.find(largestChunk, projectOnlyShard).toArray(),
                  [largestChunkShard]);
    ...
    // Remember what indexes are present, then restart the catalog on all shards via mongers.
    ...
    assert.commandWorked(mongers.adminCommand({restartCatalog: 1}));
    ...
    const secondTestDB = mongers.getDB("restart_catalog_sharded_cluster_2");
    ...
    assert.commandWorked(mongers.adminCommand({enableSharding: secondTestDB.getName()}));
    assert.commandWorked(
        mongers.adminCommand({shardCollection: foodColl.getFullName(), key: {category: 1}}));
    assert.eq(foodColl.find().toArray(), [doc]);
