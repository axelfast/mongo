// Tests mongers behavior on stale shard version errors received in a transaction.
//
...
(function() {
    ...
    const st = new ShardingTest({shards: 3, mongers: 2, config: 1});
    ...
    // Move a chunk in the first collection from Shard0 to Shard1 through the main mongers, so Shard1
    ...
    // Move a chunk in the other collection from Shard0 to Shard1 through the main mongers, so Shard1
    ...
    // Move a chunk for the other collection from Shard1 to Shard0 through the main mongers, so
    ...
    //
    // Stale mongers aborts on old shard.
    //

    // Move a chunk in the first collection from Shard1 to Shard0 through the other mongers, so
    // Shard1 and the main mongers are stale for it.
    const otherMongers = st.s1;
    assert.commandWorked(
        otherMongers.adminCommand({moveChunk: ns, find: {_id: 5}, to: st.shard0.shardName}));
    expectChunks(st, ns, [2, 0, 0]);
    ...
    // Move chunks for the first ns from Shard0 to Shard1 and Shard2 through the main mongers, so
    ...
    // Targets all shards. Shard0 is stale and won't refresh its metadata, so mongers should exhaust
