...
class CombineBenchrunEmbeddedResults(cbr.CombineBenchmarkResults):
    ...
    def before_test(self, test, test_report):
        """Remove any existing mongerebench reports for this test."""
    ...
    def _test_result_files(self, test):
        ...
        return glob.glob(
            os.path.join(self.report_root, test.short_name(), "**", "mongerebench[.]*[.]json"))

    def _parse_report_name(self, report_path):
        """Parse mongerebench report path and return thread_count.

        The format of the mongerebench report file name is defined in
        ../testing/testcases/benchrun_embedded_test.py
        as self.report_root/<test_name>/thread<num threads>/mongerebench.<iteration num>.json
        """
...
class _BenchrunEmbeddedThreadsReport(object):
    ...
    A raw mongerebench report looks like the following:
    {
