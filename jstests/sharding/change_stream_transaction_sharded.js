...
(function() {
    ...
    assert.commandWorked(mongersConn.getDB(dbName).getCollection(collName).createIndex({shard: 1}));
    ...
    assert.commandWorked(mongersConn.getDB(dbName).getCollection(collName).insert(
        {shard: 1}, {writeConcern: {w: "majority"}}));
    assert.commandWorked(mongersConn.getDB(dbName).getCollection(collName).insert(
        {shard: 2}, {writeConcern: {w: "majority"}}));

    const db = mongersConn.getDB(dbName);
    ...
    // Start transaction 1.
    //
    const session1 = db.getMonger().startSession({causalConsistency: true});
    ...
    // Start transaction 2.
    //
    const session2 = db.getMonger().startSession({causalConsistency: true});
