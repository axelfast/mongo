...
// Tests that mongerdump and mongerrestore properly handle access control information when doing
// single-db dumps and restores

// Runs the tool with the given name against the given mongerd.
...
var dumpRestoreAuth3 = function(backup_role, restore_role) {
    ...
    runTool("mongerdump", mongerd, {out: dumpDir, db: "foo", dumpDbUsersAndRoles: ""});
    db = mongerd.getDB('foo');
    ...
    runTool("mongerrestore", mongerd, {dir: dumpDir + "foo/", db: 'foo', writeConcern: "0"});
    db = mongerd.getDB('foo');
    ...
    // SERVER-23290: removed writeConcern: "0" from the mongerrestore command line options because,
    ...
    runTool("mongerrestore", mongerd, {dir: dumpDir + "foo/", db: 'foo', restoreDbUsersAndRoles: ""});
    db = mongerd.getDB('foo');
    ...
    adminUsersCount += 1;
    runTool("mongerdump", mongerd, {out: dumpDir, db: "admin"});
    db = mongerd.getDB('foo');
    ...
    runTool("mongerdump", mongerd, {out: dumpDir});
    db = mongerd.getDB('foo');
    ...
    runTool("mongerrestore", mongerd, {dir: dumpDir, writeConcern: "0"});
    db = mongerd.getDB('foo');
