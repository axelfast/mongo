...
(function() {
    ...
    load('jstests/sharding/libs/last_stable_mongers_commands.js');
    const dbName = "test";
    const collName = "foo";
    ...
    let testCases = {
        _hashBSONElement: {skip: "executes locally on mongers (not sent to any remote node)"},
        _isSelf: {skip: "executes locally on mongers (not sent to any remote node)"},
        ...
        availableQueryOptions: {skip: "executes locally on mongers (not sent to any remote node)"},
        ...
        buildInfo: {skip: "executes locally on mongers (not sent to any remote node)"},
        clearLog: {skip: "executes locally on mongers (not sent to any remote node)"},
        collMod: {
            sendsDbVersion: true,
            sendsShardVersion: true,
            setUp: function(mongersConn) {
                // Expects the collection to exist, and doesn't implicitly create it.
                assert.commandWorked(mongersConn.getDB(dbName).runCommand({create: collName}));
            },
            command: {collMod: collName},
            cleanUp: function(mongersConn) {
                assert(mongersConn.getDB(dbName).getCollection(collName).drop());
            }
        },
        collStats: {
            sendsDbVersion: true,
        ...
        compact: {skip: "not allowed through mongers"},
        configureFailPoint: {skip: "executes locally on mongers (not sent to any remote node)"},
        connPoolStats: {skip: "executes locally on mongers (not sent to any remote node)"},
        connPoolSync: {skip: "executes locally on mongers (not sent to any remote node)"},
        connectionStatus: {skip: "executes locally on mongers (not sent to any remote node)"},
        convertToCapped: {
            sendsDbVersion: true,
            sendsShardVersion: true,
            setUp: function(mongersConn) {
        ...
        features: {skip: "executes locally on mongers (not sent to any remote node)"},
        filemd5: {
            sendsDbVersion: true,
            sendsShardVersion: true,
        ...
        flushRouterConfig: {skip: "executes locally on mongers (not sent to any remote node)"},
        fsync: {skip: "broadcast to all shards"},
        getCmdLineOpts: {skip: "executes locally on mongers (not sent to any remote node)"},
        getDiagnosticData: {skip: "executes locally on mongers (not sent to any remote node)"},
        getLastError: {skip: "does not forward command to primary shard"},
        getLog: {skip: "executes locally on mongers (not sent to any remote node)"},
        getMore: {skip: "requires a previously established cursor"},
        getParameter: {skip: "executes locally on mongers (not sent to any remote node)"},
        getShardMap: {skip: "executes locally on mongers (not sent to any remote node)"},
        getShardVersion: {skip: "executes locally on mongers (not sent to any remote node)"},
        ...
        hostInfo: {skip: "executes locally on mongers (not sent to any remote node)"},
        insert: {
            sendsDbVersion: false,
            sendsShardVersion: true,
        ...
        invalidateUserCache: {skip: "executes locally on mongers (not sent to any remote node)"},
        isdbgrid: {skip: "executes locally on mongers (not sent to any remote node)"},
        isMaster: {skip: "executes locally on mongers (not sent to any remote node)"},
        ...
        listCommands: {skip: "executes locally on mongers (not sent to any remote node)"},
        ...
        logRotate: {skip: "executes locally on mongers (not sent to any remote node)"},
        ...
        mapReduce: {
            ...
            cleanUp: function(mongersConn) {
                assert(mongersConn.getDB(dbName).getCollection(collName).drop());
                assert(mongersConn.getDB(dbName).getCollection(collName + "_renamed").drop());
            }
        },
        ...
        netstat: {skip: "executes locally on mongers (not sent to any remote node)"},
        ping: {skip: "executes locally on mongers (not sent to any remote node)"},
        planCacheClear: {
            sendsDbVersion: false,
        ...
        profile: {skip: "not supported in mongers"},
        reapLogicalSessionCacheNow: {skip: "is a no-op on mongers"},
        ...
        refreshSessions: {skip: "executes locally on mongers (not sent to any remote node)"},
        refreshSessionsInternal:
            {skip: "executes locally on mongers (not sent to any remote node)", conditional: true},
        ...
        replSetGetStatus: {skip: "not supported in mongers"},
        ...
        serverStatus: {skip: "executes locally on mongers (not sent to any remote node)"},
        ...
        setFreeMonitoring:
            {skip: "explicitly fails for mongers, primary mongerd only", conditional: true},
        setParameter: {skip: "executes locally on mongers (not sent to any remote node)"},
        ...
        startRecordingTraffic: {skip: "executes locally on mongers (not sent to any remote node)"},
        startSession: {skip: "executes locally on mongers (not sent to any remote node)"},
        stopRecordingTraffic: {skip: "executes locally on mongers (not sent to any remote node)"},
        update: {
            skipProfilerCheck: true,
            sendsDbVersion: false,
    ...
    class AllCommandsTestRunner {
        ...
        runCommands() {
            ...
            // test cases that did not correspond to any mongers command.
            for (let key of Object.keys(testCases)) {
                ...
                assert(testCases[key].validated || testCases[key].conditional,
                       "you defined a test case for a command '" + key +
                           "' that does not exist on mongers: " + tojson(testCases[key]));
            }
        }
    }
