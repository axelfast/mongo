...
(function() {
    ...
    var st = new ShardingTest({shards: {rs0: {nodes: 2}}, mongers: 1});
    ...
    if (is32Bits && _isWindows()) {
        ...
        jsTest.log("Waiting for mongers to acknowledge stepdown...");

        awaitRSClientHosts(mongers,
                           secondary,
                           {ismaster: true},
                           st.rs0,
        ...
        jsTest.log("Waiting for mongers to acknowledge step up...");

        awaitRSClientHosts(mongers, primary, {ismaster: true}, st.rs0, 2 * 60 * 1000);
