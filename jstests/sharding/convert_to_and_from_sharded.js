...
(function() {
    ...
    // version suite because mongers will be 'last-stable' and if this node is of the latest binary,
    // it will report itself as the 'latest' FCV, which would cause mongers to refuse to connect to
    // it and shutdown.
    //
    ...
    var newMongerd = MongerRunner.runMongerd({shardsvr: ''});

    assert.commandWorked(st.s.adminCommand({addShard: newMongerd.name, name: 'toRemoveLater'}));
    ...
    MongerRunner.stopMongerd(newMongerd);
