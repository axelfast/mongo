#!/bin/bash
# This script downloads the mongerdb/monger-perf repository and generates mongerebench-compatible JSON
...
# We add the current working directory to the PATH because that is where the monger shell may have
# been installed. The benchrun.py script is going to run the monger shell binary that's on the PATH.
PATH=$PATH:$(pwd)

# The monger shell processes spawned by the benchrun.py script will attempt to connect to a mongerd,
# so we just error out if we find that one isn't already running.
monger --eval 'db.adminCommand({ping: 1})' && rc=$? || rc=$?
if [ "$rc" -ne 0 ]; then
    echo "This script requires a mongerd to be running on port 27017"
    exit 2
fi
...
git clone --branch=master https://github.com/mongerdb/monger-perf.git $SRC_ROOT

pushd $SRC_ROOT

# We pin to a particular commit of the mongerdb/monger-perf repository to make it clear what version
...
find testcases -type f -print0 | xargs -0 -I% -n1 -P$NUM_CPUS          \
    python2 benchrun.py --testfiles %                                  \
                        ...
                        --excludeFilter where                          \
                        --generateMongereBenchConfigFiles mongerebench/  \
                        --readCmd false                                \
                        --writeCmd true                                \
...
mv $SRC_ROOT/mongerebench $DEST_DIR
