...
#define mongerc_client_assert assert
...
struct mongerc_stream_embedded_t : mongerc_stream_t {
    monger_embedded_v1_client* clientHandle;
    MemoryView inputBuf;
...
struct FreeAndDestroy {
    void operator()(mongerc_stream_t* x) {
        auto stream = static_cast<mongerc_stream_embedded_t*>(x);
        monger_embedded_v1_client_destroy(stream->clientHandle, nullptr);
        stream->~mongerc_stream_embedded_t();
        free(stream);
    }
};
extern "C" void mongerc_stream_embedded_destroy(mongerc_stream_t* s) try {
    std::unique_ptr<mongerc_stream_t, FreeAndDestroy> stream(s);
} catch (...) {
    errno = EBADMSG;
}


extern "C" ssize_t mongerc_stream_embedded_writev(mongerc_stream_t* s,
                                                 mongerc_iovec_t* iov,
                                                 size_t iovcnt,
                                                 int32_t timeout_msec) try {
    auto stream = static_cast<mongerc_stream_embedded_t*>(s);
    mongerc_client_assert(stream->state == RPCState::kWaitingForMessageContent ||
    ...
    for (size_t i = 0; i < iovcnt; i++) {
        ...
        if (stream->state == RPCState::kWaitingForMessageLength) {
            ...
            message_length = monger::endian::littleToNative(message_length);
        ...
        mongerc_client_assert(stream->inputBuf.remaining() >= remaining_iov);
        ...
        if (stream->inputBuf.remaining() == 0) {
            ...
            int retVal = monger_embedded_v1_client_invoke(stream->clientHandle,
...
extern "C" ssize_t mongerc_stream_embedded_readv(mongerc_stream_t* s,
                                                mongerc_iovec_t* iov,
                                                size_t iovcnt,
                                                size_t min_bytes,
                                                int32_t timeout_msec) try {
    size_t bytes_read = 0;
    auto stream = static_cast<mongerc_stream_embedded_t*>(s);
    mongerc_client_assert(stream->state == RPCState::kHaveOutput);
...
extern "C" int mongerc_stream_embedded_close(mongerc_stream_t* s) {
    return 0;
}

extern "C" ssize_t mongerc_stream_embedded_poll(mongerc_stream_poll_t* s,
                                               size_t array_length,
                                               int32_t timeout_msec) try {
...
extern "C" bool mongerc_stream_embedded_check_closed(mongerc_stream_t* s) noexcept {
    return false;
}

extern "C" mongerc_stream_t* embedded_stream_initiator(const mongerc_uri_t* uri,
                                                      const mongerc_host_list_t* host,
                                                      void* user_data,
                                                      bson_error_t* error) try {
    std::unique_ptr<unsigned char, FreeDeleter> stream_buf(
        static_cast<unsigned char*>(bson_malloc0(sizeof(mongerc_stream_embedded_t))));
    if (!stream_buf) {
        errno = ENOMEM;
        return nullptr;
    }
    // Create the stream
    std::unique_ptr<mongerc_stream_embedded_t, FreeAndDestroy> stream(
        new (stream_buf.get()) mongerc_stream_embedded_t());
    stream_buf.release();  // This must be here so we don't have double ownership
    stream->state = RPCState::kWaitingForMessageLength;
    // Set up connections to database
    stream->clientHandle = monger_embedded_v1_client_create(
        static_cast<monger_embedded_v1_instance*>(user_data), nullptr);
    ...
    stream->type = 1000;
    stream->poll = mongerc_stream_embedded_poll;
    stream->close = mongerc_stream_embedded_close;
    stream->readv = mongerc_stream_embedded_readv;
    stream->writev = mongerc_stream_embedded_writev;
    stream->destroy = mongerc_stream_embedded_destroy;
    stream->check_closed = mongerc_stream_embedded_check_closed;
...
struct ClientDeleter {
    void operator()(mongerc_client_t* x) {
        mongerc_client_destroy(x);
    }
};

}  // namespace

extern "C" mongerc_client_t* MONGO_API_CALL
mongerc_embedded_v1_client_create(monger_embedded_v1_instance* db) try {
    if (!db) {
        errno = EINVAL;
        return nullptr;
    }
    std::unique_ptr<mongerc_client_t, ClientDeleter> client(mongerc_client_new(NULL));
    mongerc_client_set_stream_initiator(client.get(), embedded_stream_initiator, db);
