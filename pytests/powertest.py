...
Tests robustness of mongerd to survive multiple powercycle events.
...
def install_mongerd(bin_dir=None, tarball_url="latest", root_dir=None):
    ...
    LOGGER.debug("install_mongerd: %s %s %s", bin_dir, tarball_url, root_dir)
    # Create 'root_dir', if it does not exist.
    root_bin_dir = os.path.join(root_dir, "bin")
    if not os.path.isdir(root_dir):
        LOGGER.info("install_mongerd: creating %s", root_dir)
    ...
    if tarball_url == "latest":
        ...
        if _IS_WINDOWS:
            # MSI default:
            # https://fastdl.mongerdb.org/win32/mongerdb-win32-x86_64-2008plus-ssl-latest-signed.msi
            tarball_url = (
                "https://fastdl.mongerdb.org/win32/mongerdb-win32-x86_64-2008plus-ssl-latest.zip")
        elif _IS_LINUX:
            tarball_url = "https://fastdl.mongerdb.org/linux/mongerdb-linux-x86_64-latest.tgz"
    ...
    def stop(self, timeout):
        ...
        try:
            ...
            if ret == winerror.ERROR_BROKEN_PIPE:
                ...
                # (winerror=109) when stopping the "mongerd-powertest" service on
                # Windows Server 2016 and the underlying mongerd process has already exited.
                ret = 0
...
class MongodControl(object):  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(  # pylint: disable=too-many-arguments
        ...
        self.set_mongerd_option("dbpath", db_path)
        self.log_path = log_path
        self.set_mongerd_option("logpath", log_path)
        self.set_mongerd_option("logappend")
        self.port = port
        self.set_mongerd_option("port", port)
        self.set_mongerd_option("bind_ip", "0.0.0.0")
        if _IS_WINDOWS:
            self.set_mongerd_option("service")
            self._service = WindowsService
        else:
            self.set_mongerd_option("fork")
            self._service = PosixService
        # After mongerd has been installed, self.bin_path is defined.
        if self.bin_path:
            self.service = self._service("mongerd-powertest", self.bin_path, self.mongerd_options())

    def set_mongerd_option(self, option, option_value=None, option_form="--"):
        """Set mongerd command line option."""
        self.options_map[option] = (option_value, option_form)

    def get_mongerd_option(self, option):
        """Return tuple of (value, form)."""
        return self.options_map[option]

    def get_mongerd_service(self):
        """Return the service object used to control mongerd."""
        return self.service

    def mongerd_options(self):
        """Return string of mongerd options, which can be used when invoking mongerd."""
        opt_string = ""
    ...
    def install(self, root_dir, tarball_url):
        ...
        # Install mongerd, if 'root_dir' does not exist.
        if os.path.isdir(root_dir):
            LOGGER.warning("Root dir %s already exists", root_dir)
        else:
            install_mongerd(bin_dir=self.bin_dir, tarball_url=tarball_url, root_dir=root_dir)
        ...
        # is only known after install_mongerd runs.
        self.service = self._service("mongerd-powertest", self.bin_path, self.mongerd_options())
...
def remote_handler(options, operations):  # pylint: disable=too-many-branches,too-many-locals,too-many-statements
    ...
    for operation in operations:
        ...
        # This is the internal "crash" mechanism, which is executed on the remote host.
        ...
        elif operation == "kill_mongerd":
            # Unconditional kill of mongerd.
            ret, output = kill_mongerd()
            if ret:
                LOGGER.error("kill_mongerd failed %s", output)
                return ret
            # Ensure the mongerd service is not in a running state.
            mongerd.stop(timeout=30)
            status = mongerd.status()
            if status != "stopped":
                LOGGER.error("Unable to stop the mongerd service, in state '%s'", status)
                ret = 1

        elif operation == "install_mongerd":
            ret, output = mongerd.install(root_dir, options.tarball_url)
            LOGGER.info(output)

            # Create mongerd's dbpath, if it does not exist.
            if not os.path.isdir(db_path):
                os.makedirs(db_path)

            # Create mongerd's logpath directory, if it does not exist.
        ...
        elif operation == "start_mongerd":
            # Always update the service before starting, as options might have changed.
            ret, output = mongerd.update()
            LOGGER.info(output)
            ret, output = mongerd.start()
            LOGGER.info(output)
            if ret:
            ...
            if options.repl_set:
                ret = monger_reconfig_replication(monger, host_port, options.repl_set)

        elif operation == "stop_mongerd":
            ret, output = mongerd.stop()
            LOGGER.info(output)
            ...
            lock_file = os.path.join(options.db_path, "mongerd.lock")
            if os.path.exists(lock_file):
                LOGGER.debug("Deleting mongerd lockfile %s", lock_file)
                try:
                    os.remove(lock_file)
                except (IOError, OSError) as err:
                    LOGGER.warning("Unable to delete mongerd lockfile %s with error %s", lock_file,
                                   err)
                    ret = err.code
...
def kill_mongerd():
    """Kill all mongerd processes uncondtionally."""
    if _IS_WINDOWS:
        cmds = "taskkill /f /im mongerd.exe"
    else:
        cmds = "pkill -9 mongerd"
    ret, output = execute_cmd(cmds, use_file=True)
    return ret, output
...
def crash_server_or_kill_mongerd(  # pylint: disable=too-many-arguments,,too-many-locals
        options, crash_canary, canary_port, local_ops, script_name, client_args):
    """Crash server or kill mongerd and optionally write canary doc. Return tuple (ret, output)."""

    crash_wait_time = options.crash_wait_time + random.randint(0, options.crash_wait_time_jitter)
    message_prefix = "Killing mongerd" if options.crash_method == "kill" else "Crashing server"
    ...
    if options.crash_method == "mpower":
        ...
        crash_cmd = "crash_server" if options.crash_method == "internal" else "kill_mongerd"
        if options.canary == "remote":
            # The crash canary function executes remotely, only if the
            # crash_method is 'internal'.
            canary = "--mongerdPort {} --docForCanary \"{}\"".format(canary_port,
                                                                    crash_canary["args"][3])
...
def wait_for_mongerd_shutdown(mongerd_control, timeout=120):
    """Wait for for mongerd to shutdown; return 0 if shutdown occurs within 'timeout', else 1."""
    start = time.time()
    status = mongerd_control.status()
    while status != "stopped":
        if time.time() - start >= timeout:
            LOGGER.error("The mongerd process has not stopped, current status is %s", status)
            return 1
        LOGGER.info("Waiting for mongerd process to stop, current status is %s ", status)
        time.sleep(3)
        status = mongerd_control.status()
    LOGGER.info("The mongerd process has stopped")
...
def get_monger_client_args(host=None, port=None, options=None, server_selection_timeout_ms=600000,
    ...
    if host:
        monger_args["host"] = host
    if port:
        monger_args["port"] = port
    # Set the writeConcern
    if hasattr(options, "write_concern"):
        monger_args.update(yaml.safe_load(options.write_concern))
    # Set the readConcernLevel
    if hasattr(options, "read_concern_level") and options.read_concern_level:
        monger_args["readConcernLevel"] = options.read_concern_level
    return monger_args


def monger_shell(  # pylint: disable=too-many-arguments
        monger_path, work_dir, host_port, monger_cmds, retries=5, retry_sleep=5):
    """Start monger_path from work_dir, connecting to host_port and executes monger_cmds."""
    cmds = "cd {}; echo {} | {} {}".format(
        pipes.quote(work_dir), pipes.quote(monger_cmds), pipes.quote(monger_path), host_port)
    attempt_num = 0
    while True:
        ret, output = execute_cmd(cmds, use_file=True)
        if not ret:
...
def mongerd_wait_for_primary(monger, timeout=60, sleep_interval=3):
    """Return True if mongerd primary is available in replica set, within the specified timeout."""

    start = time.time()
    while not monger.admin.command("isMaster")["ismaster"]:
...
def monger_reconfig_replication(monger, host_port, repl_set):
    """Reconfigure the mongerd replica set. Return 0 if successful."""

    # TODO: Rework reconfig logic as follows:
    # 1. Start up mongerd in standalone
    ...
    database = pymonger.database.Database(monger, "local")
    ...
    if not system_replset or not system_replset.find_one():
        ...
        ret = monger.admin.command("replSetInitiate", rs_config)
        LOGGER.info("Replication initialized: %s %s", ret, rs_config)
    else:
        # Wait until replication is initialized.
        while True:
            try:
                ret = monger.admin.command("replSetGetConfig")
                if ret["ok"] != 1:
                ...
                if rs_config["members"][0]["host"] != host_port:
                    ...
                    ret = monger.admin.command("replSetReconfig", rs_config, force=True)
                    if ret["ok"] != 1:
                ...
                # src/monger/base/error_codes.err: error_code("NotYetInitialized", 94)
                if err.code != 94:
                    LOGGER.error("Replication failed to initialize: %s", ret)
                    return 1

    primary_available = mongerd_wait_for_primary(monger)
    LOGGER.debug("isMaster: %s", monger.admin.command("isMaster"))
    LOGGER.debug("replSetGetStatus: %s", monger.admin.command("replSetGetStatus"))
    return 0 if ret["ok"] == 1 and primary_available else 1


def monger_seed_docs(monger, db_name, coll_name, num_docs):
    ...
    LOGGER.info("Seeding DB '%s' collection '%s' with %d documents, %d already exist", db_name,
                coll_name, num_docs, monger[db_name][coll_name].count())
    random.seed()
    base_num = 100000
    bulk_num = min(num_docs, 10000)
    bulk_loops = num_docs // bulk_num
    for _ in range(bulk_loops):
        num_coll_docs = monger[db_name][coll_name].count()
        if num_coll_docs >= num_docs:
            break
        monger[db_name][coll_name].insert_many(
    ...
    LOGGER.info("After seeding there are %d documents in the collection",
                monger[db_name][coll_name].count())
    return 0


def monger_validate_collections(monger):
    """Validate the monger collections, return 0 if all are valid."""

    LOGGER.info("Validating all collections")
    invalid_colls = []
    ebusy_colls = []
    for db_name in monger.database_names():
        for coll in monger[db_name].list_collections(filter={"type": "collection"}):
            coll_name = coll["name"]
            res = monger[db_name].command({"validate": coll_name, "full": True})
...
def monger_validate_canary(monger, db_name, coll_name, doc):
    ...
    return 0 if monger[db_name][coll_name].find_one(doc) else 1


def monger_insert_canary(monger, db_name, coll_name, doc):
    ...
    coll = monger[db_name][coll_name].with_options(
        write_concern=pymonger.write_concern.WriteConcern(j=True))
...
def resmoke_client(  # pylint: disable=too-many-arguments
        work_dir, monger_path, host_port, js_test, resmoke_suite, repeat_num=1, no_wait=False,
        log_file=None):
    ...
    cmds = ("cd {}; "
            ...
            " --monger {}"
            " --suites {}"
            " --shellConnString mongerdb://{}"
            " --continueOnFailure"
            " --repeat {}"
            " {}"
            " {}".format(
                pipes.quote(work_dir), pipes.quote(monger_path), pipes.quote(resmoke_suite),
...
Examples:
    ...
    Server is running as single node replica set connected to mFi mPower, outlet1:
      python powertest.py
            ...
            --mongerdOptions "--storageEngine mmapv1"
...
""")
    ...
    test_options.add_option("--rsync", dest="rsync_data",
                            help="Rsync data directory between mongerd stop and start",
    ...
    test_options.add_option(
        ...
        help="Run validate on all collections after mongerd restart after"
    ...
    test_options.add_option(
        "--writeConcern", dest="write_concern", help="monger (shell) CRUD client writeConcern, i.e.,"
        " '{\"w\": \"majority\"}' [default: '%default']", default="{}")

    test_options.add_option("--readConcernLevel", dest="read_concern_level",
                            help="monger (shell) CRUD client readConcernLevel, i.e.,"
    ...
    crash_options.add_option(
        ...
        " Select 'kill' to perform an unconditional kill of mongerd,"
    ...
    mongerdb_options.add_option(
        ...
        " directory [default: 'mongerdb-powertest-<epochSecs>']", default=None)

    mongerdb_options.add_option(
        "--mongerdbBinDir", dest="mongerdb_bin_dir",
        help="Directory, on remote host, containing mongerDB binaries,"
    ...
    mongerdb_options.add_option(
        ...
        " it will be '<rootDir>/log/mongerd.log'", default=None)

    # mongerd options
    mongerd_options.add_option(
        "--replSet", dest="repl_set",
        help="Name of mongerd single node replica set, if unpsecified mongerd"
        " defaults to standalone node", default=None)

    # The current host used to start and connect to mongerd. Not meant to be specified
    # by the user.
    mongerd_options.add_option("--mongerdHost", dest="host", help=optparse.SUPPRESS_HELP,
                              default=None)

    # The current port used to start and connect to mongerd. Not meant to be specified
    # by the user.
    mongerd_options.add_option("--mongerdPort", dest="port", help=optparse.SUPPRESS_HELP, type="int",
                              default=None)

    # The ports used on the 'server' side when in standard or secret mode.
    mongerd_options.add_option(
        "--mongerdUsablePorts", dest="usable_ports", nargs=2,
        help="List of usable ports to be used by mongerd for"
        " standard and secret modes, [default: %default]", type="int", default=[27017, 37017])

    mongerd_options.add_option("--mongerdOptions", dest="mongerd_options",
                              help="Additional mongerd options", default="")

    mongerd_options.add_option("--fcv", dest="fcv_version",
                              help="Set the FeatureCompatibilityVersion of mongerd.", default=None)

    mongerd_options.add_option(
        "--removeLockFile", dest="remove_lock_file",
        help="If specified, the mongerd.lock file will be deleted after a"
        " powercycle event, before mongerd is started. This is a"
        " workaround for mongerd failing start with MMAPV1 (See"
        " SERVER-15109).", action="store_true", default=False)

    # Client options
    monger_path = distutils.spawn.find_executable("monger",
                                                 os.getcwd() + os.pathsep + os.environ["PATH"])
    client_options.add_option(
        "--mongerPath", dest="monger_path",
        help="Path to monger (shell) executable, if unspecifed, monger client"
        " is launched from the current directory.", default=monger_path)

    client_options.add_option(
        "--mongerRepoRootDir", dest="monger_repo_root_dir",
        help="Root directory of mongerDB repository, defaults to current"
    ...
    client_options.add_option(
        ...
        " provides a monger (shell) DB connection to a running mongerd.",
    ...
    parser.add_option_group(mongerdb_options)
    parser.add_option_group(mongerd_options)
    ...
    # Initialize the mongerd options
    # Note - We use posixpath for Windows client to Linux server scenarios.
    if not options.root_dir:
        options.root_dir = "mongerdb-powertest-{}".format(int(time.time()))
    if not options.db_path:
        options.db_path = posixpath.join(options.root_dir, "data", "db")
    if not options.log_path:
        options.log_path = posixpath.join(options.root_dir, "log", "mongerd.log")
    mongerd_options_map = parse_options(options.mongerd_options)
    ...
    # Setup the monger client, monger_path is required if there are local clients.
    if (options.num_crud_clients > 0 or options.num_fsm_clients > 0
            or options.validate_collections == "local"):
        if not options.monger_path:
            LOGGER.error("mongerPath must be specified")
            local_exit(1)
        if not os.path.isfile(options.monger_path):
            LOGGER.error("mongerPath %s does not exist", options.monger_path)
            local_exit(1)
        monger_path = os.path.abspath(os.path.normpath(options.monger_path))
    ...
    # Setup the monger_repo_root.
    if options.monger_repo_root_dir:
        monger_repo_root_dir = options.monger_repo_root_dir
    else:
        monger_repo_root_dir = os.getcwd()
    if not os.path.isdir(monger_repo_root_dir):
        LOGGER.error("mongerRepoRoot %s does not exist", monger_repo_root_dir)
        local_exit(1)
    ...
    if options.canary and "nojournal" in mongerd_options_map:
        LOGGER.error("Cannot create and validate canary documents if the mongerd option"
                     " '--nojournal' is used.")
        local_exit(1)
    ...
    # Set the Pymonger connection timeout to 1 hour for canary insert & validation.
    one_hour_ms = 60 * 60 * 1000

    # The remote mongerd host comes from the ssh_user_host,
    # which may be specified as user@host.
    ssh_user_host = options.ssh_user_host
    ssh_user, ssh_host = get_user_host(ssh_user_host)
    mongerd_host = ssh_host
    ...
    #   2. Start mongerd on the secret port and wait for it to recover
    #   3  Validate collections (optional)
    #   4. Validate canary (optional)
    #   5. Stop mongerd
    #   6. Rsync the database (optional, post-recovery)
    #   7. Start mongerd on the standard port
    #   8. Start monger (shell) & FSM clients
    ...
    while True:
        ...
        remote_operation = ("--remoteOperation"
                            ...
                            " {validate_canary_cmd}"
                            " {seed_docs}").format(
                                rsync_opt=rsync_opt, canary_opt=canary_opt, host=mongerd_host,
        ...
        LOGGER.info("****%sstart mongerd: %d %s****", rsync_text, ret, output)
        if ret:
            local_exit(ret)
        ...
        if options.validate_collections == "local":
            host_port = "{}:{}".format(mongerd_host, secret_port)
            ...
            ret, output = resmoke_client(monger_repo_root_dir, monger_path, host_port,
        ...
        # Shutdown mongerd on secret port.
        remote_op = ("--remoteOperation" " --mongerdPort {}" " shutdown_mongerd").format(secret_port)
        ret, output = call_remote_operation(local_ops, options.remote_python, script_name,
                                            client_args, remote_op)
        LOGGER.info("****shutdown_mongerd: %d %s****", ret, output)
        if ret:
            local_exit(ret)
        ...
        remote_op = ("--remoteOperation"
                     ...
                     " --mongerdPort {}"
                     " {}"
                     " start_mongerd").format(rsync_opt, mongerd_host, standard_port, rsync_cmd)
        ret, output = call_remote_operation(local_ops, options.remote_python, script_name,
                                            client_args, remote_op)
        rsync_text = "rsync_data afterrecovery & " if options.rsync_data else ""
        LOGGER.info("****%s start mongerd: %d %s****", rsync_text, ret, output)
        if ret:
            local_exit(ret)
        ...
        # Start CRUD clients
        host_port = "{}:{}".format(mongerd_host, standard_port)
        for i in range(options.num_crud_clients):
            ...
            _, _ = resmoke_client(work_dir=monger_repo_root_dir, monger_path=monger_path,
        ...
        if options.canary:
            ...
            crash_canary["args"] = [monger, options.db_name, options.collection_name, canary_doc]
        ret, output = crash_server_or_kill_mongerd(options, crash_canary, standard_port, local_ops,
                                                  script_name, client_args)

        LOGGER.info("Crash server or Kill mongerd: %d %s****", ret, output)
