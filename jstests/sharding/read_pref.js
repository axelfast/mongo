...
(function() {
    ...
    var doTest = function(useDollarQuerySyntax) {
        ...
        if (useDollarQuerySyntax && st.s.getDB("test").getMonger().useReadCommands()) {
            st.stop();
            return;
        }
        ...
        replConfig.members.forEach(function(node) {
            var nodeConn = new Monger(node.host);
        ...
        // Check that mongers will try the next tag if nothing matches the first
        explain = getExplain("secondary", [{z: "3"}, {dc: "jp"}]);
        explainServer = getExplainServer(explain);
        checkTag(explainServer, {dc: "jp"});
        assert.eq(1, explain.executionStats.nReturned);

        // Check that mongers will fallback to primary if none of tags given matches
