...
(function() {
    ...
    var st = new ShardingTest({shards: 2, mongers: 1, rs: {nodes: 1}});

    const dbName = "testDB";
    const ns = dbName + ".foo";

    let mongers = st.s0;
    let admin = mongers.getDB('admin');
    let coll = mongers.getCollection(ns);
    ...
    var joinMoveChunk = moveChunkParallel(
        staticMongerd, st.s0.host, {_id: 2}, null, coll.getFullName(), st.shard1.shardName);
