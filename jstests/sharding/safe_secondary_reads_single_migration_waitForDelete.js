...
(function() {
    ...
    for (let command of commands) {
        ...
        // We do this because we expect freshMongers to see that the collection is sharded, which it
        ...
        assert.commandWorked(staleMongers.adminCommand({split: nss, middle: {x: 0}}));
        ...
        let res = staleMongers.getDB(db).runCommand(Object.extend(
        ...
        if (test.behavior === "unshardedOnly") {
            ...
            // shardVersion once, even though the mongers is fresh, because the secondary was
            // stale.
            profilerHasSingleMatchingEntryOrThrow({
                profileDB: donorShardSecondary.getDB(db),
                filter: Object.extend({
