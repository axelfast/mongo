...
(function() {
    ...
    var st = new ShardingTest({name: "remove_shard3", shards: 2, mongers: 2});
    ...
    // Make sure both mongers instances know of the latest metadata
    ...
    // Make sure both mongers instance refresh their metadata and do not reference the missing shard
    assert.eq(2, st.s0.getDB('TestDB').Coll.find({}).toArray().length);
    assert.eq(2, st.s1.getDB('TestDB').Coll.find({}).toArray().length);
