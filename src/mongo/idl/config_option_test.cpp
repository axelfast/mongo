...
#include "monger/idl/config_option_no_init_test_gen.h"
#include "monger/idl/config_option_test_gen.h"
...
TEST(ConfigOption, Opt1) {
    ...
    ASSERT_OK(parseArgv({"mongerd", "--testConfigOpt1"}, &parsed));
...
TEST(ConfigOption, Opt2) {
    ...
    ASSERT_OK(parseArgv({"mongerd"}, &parsedAbsent));
    ASSERT_OPTION_NOT_SET<bool>(parsedAbsent, "test.config.opt2");

    moe::Environment parsedTrue;
    ASSERT_OK(parseArgv({"mongerd", "--testConfigOpt2", "true"}, &parsedTrue));
    ASSERT_OPTION_SET<bool>(parsedTrue, "test.config.opt2", true);

    moe::Environment parsedFalse;
    ASSERT_OK(parseArgv({"mongerd", "--testConfigOpt2", "false"}, &parsedFalse));
    ASSERT_OPTION_SET<bool>(parsedFalse, "test.config.opt2", false);

    moe::Environment parsedFail;
    ASSERT_NOT_OK(parseArgv({"mongerd", "--testConfigOpt2"}, &parsedFail));
    ASSERT_NOT_OK(parseArgv({"mongerd", "--testConfigOpt2", "banana"}, &parsedFail));
...
TEST(ConfigOption, Opt3) {
    ...
    moe::Environment parsedTrue;
    ASSERT_OK(parseArgv({"mongerd", "--testConfigOpt3", "true"}, &parsedTrue));
    ASSERT_OPTION_SET<bool>(parsedTrue, "test.config.opt3", true);

    moe::Environment parsedFalse;
    ASSERT_OK(parseArgv({"mongerd", "--testConfigOpt3", "false"}, &parsedFalse));
    ASSERT_OPTION_SET<bool>(parsedFalse, "test.config.opt3", false);

    moe::Environment parsedImplicit;
    ASSERT_OK(parseArgv({"mongerd", "--testConfigOpt3"}, &parsedImplicit));
...
TEST(ConfigOption, Opt5) {
    ...
    moe::Environment parsedFail;
    ASSERT_NOT_OK(parseArgv({"mongerd", "--testConfigOpt5"}, &parsedFail));
    ASSERT_NOT_OK(parseArgv({"mongerd", "--testConfigOpt5", "123"}, &parsedFail));
...
TEST(ConfigOption, Opt6) {
    ...
    moe::Environment parsed;
    ASSERT_OK(parseArgv({"mongerd", "some value"}, &parsed));
...
TEST(ConfigOption, Opt7) {
    ...
    ASSERT_OK(parseArgv({"mongerd", "value1"}, &parsedSingleArg));
    ...
    ASSERT_OK(parseArgv({"mongerd", "value1", "value2", "value3"}, &parsedMultiArg));
...
TEST(ConfigOption, Opt8) {
    ...
    moe::Environment parsed;
    ASSERT_OK(parseArgv({"mongerd", "--testConfigOpt8", "42"}, &parsed));
    ASSERT_OPTION_SET<long>(parsed, "test.config.opt8", 42);

    moe::Environment parsedDeprShort;
    ASSERT_OK(parseArgv({"mongerd", "--testConfigOpt8a", "43"}, &parsedDeprShort));
...
TEST(ConfigOption, Opt9) {
    ...
    ASSERT_OK(
        parseArgv({"mongerd", "--testConfigOpt9", "42", "--testConfigOpt9a", "43"}, &parsedCLI));
    ...
    ASSERT_OK(parseMixed(
        {"mongerd", "--testConfigOpt9", "42"}, "test: { config: { opt9a: 43 } }", &parsedMixed));
    ...
    moe::Environment parsedFail;
    ASSERT_NOT_OK(parseArgv({"mongerd", "--testConfigOpt9", "42"}, &parsedFail));
    ASSERT_NOT_OK(
        parseArgv({"mongerd", "--testConfigOpt9", "42", "--testConfigOpt9b", "44"}, &parsedFail));
    ASSERT_NOT_OK(parseArgv(
        {"mongerd", "--testConfigOpt9", "42", "--testConfigOpt9a", "43", "--testConfigOpt9b", "44"},
        &parsedFail));
...
TEST(ConfigOption, Opt10) {
    ...
    const auto failParse = [](int a, int b) {
        moe::Environment parsedFail;
        ASSERT_NOT_OK(parseArgv({"mongerd",
                                 "--testConfigOpt10a",
                                 std::to_string(a),
                                 "--testConfigOpt10b",
                                 std::to_string(b)},
...
TEST(ConfigOption, Opt11) {
    ...
    const auto tryParse = [](int val) {
        moe::Environment parsed;
        ASSERT_OK(parseArgv({"mongerd", "--testConfigOpt11", std::to_string(val)}, &parsed));
        ASSERT_OPTION_SET<int>(parsed, "test.config.opt11", val);
    };
    const auto failParse = [](int val) {
        moe::Environment parsed;
        ASSERT_NOT_OK(parseArgv({"mongerd", "--testConfigOpt11", std::to_string(val)}, &parsed));
    };
    tryParse(1);
    tryParse(123456789);
    failParse(0);
...
TEST(ConfigOption, Opt13) {
    ...
    ASSERT_OK(parseArgv({"mongerd", "-o", "single"}, &parsedSingle));
    ASSERT_OPTION_SET<std::string>(parsedSingle, "test.config.opt13", "single");

    moe::Environment parsedShort;
    ASSERT_OK(parseArgv({"mongerd", "--testConfigOpt13", "short"}, &parsedShort));
...
TEST(ConfigOption, Opt15) {
    ...
    ASSERT_NOT_OK(parseArgv({"mongerd", "--testConfigOpt15", "set15"}, &parseFail));
...
TEST(ConfigOption, Opt17) {
    ...
    ASSERT_OK(parseArgv({"mongerd", "--testConfigOpt17"}, &implicitParse));
    ASSERT_OPTION_SET<std::int32_t>(implicitParse, "test.config.opt17", kTestConfigOpt17Implicit);

    moe::Environment negativeParse;
    ASSERT_NOT_OK(
        parseArgv({"mongerd", "--testConfigOpt17", std::to_string(kTestConfigOpt17Minimum - 1)},
                  &negativeParse));
    ASSERT_NOT_OK(
        parseArgv({"mongerd", "--testConfigOpt17", std::to_string(kTestConfigOpt17Maximum + 1)},
                  &negativeParse));

    moe::Environment okParse;
    ASSERT_OK(parseArgv({"mongerd", "--testConfigOpt17", std::to_string(kTestConfigOpt17Minimum)},
                        &okParse));
...
TEST(ConfigOptionNoInit, Opt1) {
    ...
    const std::vector<std::string> argv({
        "mongerd", "--testConfigNoInitOpt1", "Hello",
    });
    moe::Environment parsed;
