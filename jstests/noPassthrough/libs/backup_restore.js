...
var BackupRestoreTest = function(options) {
    ...
    function _crudClient(host, dbName, collectionName, numNodes) {
        ...
        // Returns the pid of the started monger shell so the CRUD test client can be terminated
        // without waiting for its execution to finish.
        return startMongerProgramNoConnect(MongerRunner.mongerShellPath,
                                          '--eval',
    ...
    function _fsmClient(host) {
        ...
        const resmokeCmd = 'python buildscripts/resmoke.py --shuffle --continueOnFailure' +
            ' --repeat=99999 --monger=' + MongerRunner.mongerShellPath +
            ' --shellConnString=mongerdb://' + host + ' --suites=' + suite;

        // Returns the pid of the FSM test client so it can be terminated without waiting for its
        // execution to finish.
        return _startMongerProgram({args: resmokeCmd.split(' ')});
    }
    ...
    this.run = function() {
        ...
        var dbpathPrefix = MongerRunner.dataPath + 'backupRestore';
        resetDbpath(dbpathPrefix);
        var dbpathFormat = dbpathPrefix + '/mongerd-$port';
        ...
        var hiddenDbpath = dbpathPrefix + '/mongerd-hiddensecondary';
        ...
        if (options.backup == 'fsyncLock') {
            ...
            removeFile(hiddenDbpath + '/mongerd.lock');
        ...
        assert(crudStatus.alive,
               testName + ' CRUD client was not running at end of test and exited with code: ' +
                   crudStatus.exitCode);
        stopMongerProgramByPid(crudPid);
        ...
        const kSIGINT = 2;
        const exitCode = stopMongerProgramByPid(fsmPid, kSIGINT);
        if (!_isWindows()) {
            // The monger shell calls TerminateProcess() on Windows rather than more gracefully
