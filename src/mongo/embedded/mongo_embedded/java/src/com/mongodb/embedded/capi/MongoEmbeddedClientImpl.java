...
class MongerEmbeddedClientImpl implements MongerEmbeddedClient {
    ...
    MongerEmbeddedClientImpl(final CAPI.monger_embedded_v1_instance instance) {
        ...
        try {
            client = CAPI.monger_embedded_v1_client_create(instance, status);
        } catch (Throwable t) {
    ...
    public void close() {
        try {
            CAPIHelper.validateErrorCode(status,
                    CAPI.monger_embedded_v1_client_destroy(client, status));
        } catch (Throwable t) {
    ...
    public ByteBuffer write(final ByteBuffer buffer) {
        ...
        try {
            CAPIHelper.validateErrorCode(status,
                    CAPI.monger_embedded_v1_client_invoke(client, messagePointer, new NativeLong(message.length), outputBufferReference,
                            outputSize, status));
        } catch (Throwable t) {
