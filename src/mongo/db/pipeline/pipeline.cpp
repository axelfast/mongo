...
void Pipeline::detachFromOperationContext() {
    pCtx->opCtx = nullptr;
    pCtx->mongerProcessInterface->setOperationContext(nullptr);
...
void Pipeline::reattachToOperationContext(OperationContext* opCtx) {
    pCtx->opCtx = opCtx;
    pCtx->mongerProcessInterface->setOperationContext(opCtx);
...
bool Pipeline::requiredToRunOnMongers() const {
    ...
    for (auto&& stage : _sources) {
        // If this pipeline is capable of splitting before the mongerS-only stage, then the pipeline
        // as a whole is not required to run on mongerS.
        ...
        if (hostRequirement == HostTypeRequirement::kMongerS) {
            ...
            uassertStatusOKWithContext(mongersRunStatus,
                                       str::stream() << stage->getSourceName()
                                                     << " must run on mongerS, but cannot");
...
Status Pipeline::_pipelineCanRunOnMongerS() const {
    for (auto&& stage : _sources) {
        ...
        if (needsShard) {
            ...
            ss << " is a blocking stage; running these stages on mongerS is disabled";
