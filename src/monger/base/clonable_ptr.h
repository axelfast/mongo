...
// This is the default `CloneFactory` conforming to `monger::concept::CloneFactory` for
// `clonable_ptr`.
 ...
 * `CloneFactory` type for a specific type.  Types which conform to `monger::concept::Clonable`
 ...
 * type which models the Concept `monger::concept::Clonable`.  The supplied type may supply an
 * alternative default `CloneFactory` type by either of two means:
 *
 *  * `T` may define a member `T::clone_factory_type` which conforms to
 *    `monger::concept::CloneFactory`
 *  * `T` may have an accompanying specialization of `monger::clonable_traits< T >` which
 ...
 * `CloneFactory`: A type which models the Concept `monger::concept::CloneFactory`.
 * `UniquePtr`: A type which models the Concept `monger::concept::UniquePtr`
 */
template <typename T,
