/**
 * Run a query on a sharded cluster where one of the shards hangs. Running killCursors on the mongers
...
(function() {
    ...
    const kDBName = "test";
    const mongersDB = st.s.getDB(kDBName);
    const shard0DB = st.shard0.getDB(kDBName);
    const shard1DB = st.shard1.getDB(kDBName);

    let coll = mongersDB.jstest_kill_pinned_cursor;
    coll.drop();
    ...
    //
    // Sets up a sharded cursor, opens a mongers cursor, and uses failpoints to cause the mongers
    // cursor to hang due to getMore commands hanging on each of the shards. Then invokes
    // 'killFunc', and verifies the cursors on the shards and the mongers cursor get cleaned up.
    //
    ...
    function testShardedKillPinned(
        ...
        try {
            // Set up the mongerds to hang on a getMore request. ONLY set the failpoint on the
            // mongerds. Setting the failpoint on the mongers will only cause it to spin, and not
            ...
            // Run a find against mongers. This should open cursors on both of the shards.
            let findCmd = {find: coll.getName(), batchSize: 2};

            if (useSession) {
                // Manually start a session so it can be continued from inside a parallel shell.
                sessionId = assert.commandWorked(mongersDB.adminCommand({startSession: 1})).id;
                findCmd.lsid = sessionId;
            }

            let cmdRes = mongersDB.runCommand(findCmd);
            ...
            // Sleep until we know the mongerd cursors are pinned.
            ...
            // By now, the getMore run against the mongers has returned with an indication that the
            ...
            let killRes = mongersDB.runCommand({killCursors: coll.getName(), cursors: [cursorId]});
            ...
            // Eventually the cursors on the mongerds should also be cleaned up. They should be
            // killed by mongers when the mongers cursor gets killed.
            assert.soon(() => shard0DB.serverStatus().metrics.cursor.open.pinned == 0);
            assert.soon(() => shard1DB.serverStatus().metrics.cursor.open.pinned == 0);
    ...
    for (let useSession of[true, false]) {
        // Test that running 'killCursors' against a pinned mongers cursor (with pinned mongerd
        // cursors) correctly cleans up all of the involved cursors.
        testShardedKillPinned({
            killFunc: function(mongersCursorId) {
                // Run killCursors against the mongers cursor. Verify that the cursor is reported as
                // killed successfully, and does not hang or return a "CursorInUse" error.
                let cmdRes =
                    mongersDB.runCommand({killCursors: coll.getName(), cursors: [mongersCursorId]});
                assert.commandWorked(cmdRes);
                assert.eq(cmdRes.cursorsKilled, [mongersCursorId]);
        ...
        // Test that running killOp against one of the cursors pinned on mongerd causes all involved
        // cursors to be killed.
        testShardedKillPinned({
            // This function ignores the mongers cursor id, since it instead uses currentOp to
        ...
        // Test that running killCursors against one of the cursors pinned on mongerd causes all
    ...
    // cursor to be killed.
    testShardedKillPinned({
        // This function ignores the mongers cursor id, since it instead uses listLocalSessions
        ...
        killFunc: function() {
            ...
            assert.commandWorked(mongersDB.runCommand({killSessions: [{id: sessionUUID}]}));
        },
        // Killing a session on mongers kills all matching remote cursors (through KillCursors) then
