// Tests that the $changeStream stage returns an error when run against a standalone mongerd.
...
(function() {
    ...
    const conn = MongerRunner.runMongerd({enableMajorityReadConcern: ""});
    ...
    assert.eq(0, MongerRunner.stopMongerd(conn));
    ...
    const mongersDB = clusterWithStandalones.getDB("test");
    // enableSharding will create the db at the cluster level but not on the shards. $changeStream
    // through mongerS will be allowed to run on the shards despite the lack of a database.
    assert.commandWorked(mongersDB.adminCommand({enableSharding: "test"}));
    ...
    assert.commandWorked(
        mongersDB.adminCommand({shardCollection: "test.ensure_db_exists", key: {_id: "hashed"}}));
