/**
 * Tests that mongers does not gossip cluster time metadata until at least one key is created on the
...
(function() {
    ...
    // Verify there are keys in the config server eventually, since mongers doesn't block for keys at
    // startup, and that once there are, mongers sends $clusterTime with a signature in responses.
    assert.soonNoExcept(function() {
        ...
        return true;
    }, "expected keys to be created and for mongers to send signed cluster times");
    ...
    assert.soonNoExcept(function() {
        ...
        return true;
    }, "expected mongers to eventually start signing cluster times", 60 * 1000);  // 60 seconds.

    // There may be a delay between the creation of the first and second keys, but mongers will start
