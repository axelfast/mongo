/**
 * This tests that updates to user and role definitions made on one mongers propagate properly
 * to other mongerses.
 ...
 * different mongerses.  "db1", "db2", and "db3" are all auth'd as spencer@test and will be used
 * to verify that user and role data changes get propaged to their mongerses.
 * "db2" is connected to a mongers with a 5 second user cache invalidation interval,
 * while "db3" is connected to a mongers with a 10 minute cache invalidation interval.
 */
...
(function testGrantingPrivileges() {
    ...
    }, "Mongers did not update its user cache after 10 seconds", 10 * 1000);
...
(function testConcurrentUserModification() {
    jsTestLog("Testing having 2 mongerses modify the same user at the same time");  // SERVER-13850
...
print("SUCCESS Completed mongers_cache_invalidation.js");
