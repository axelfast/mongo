// Tests snapshot isolation on readConcern level snapshot reads through mongers.
...
(function() {
    ...
    let shardingScenarios = {
        ...
        singleShard: {
            ...
            setUp: function(collName) {
                const st = new ShardingTest({shards: 1, mongers: 1, config: 1});
        ...
        multiShardAllShardReads: {
            ...
            setUp: function(collName) {
                let st = new ShardingTest({shards: 3, mongers: 1, config: 1});
                ...
                assert.commandWorked(
                    mongers.adminCommand({moveChunk: ns, find: {_id: 0}, to: st.shard0.shardName}));
                assert.commandWorked(
                    mongers.adminCommand({moveChunk: ns, find: {_id: 4}, to: st.shard1.shardName}));
                assert.commandWorked(
                    mongers.adminCommand({moveChunk: ns, find: {_id: 7}, to: st.shard2.shardName}));

                assert.eq(
                    1, mongers.getDB('config').chunks.count({ns: ns, shard: st.shard0.shardName}));
                assert.eq(
                    1, mongers.getDB('config').chunks.count({ns: ns, shard: st.shard1.shardName}));
                assert.eq(
                    1, mongers.getDB('config').chunks.count({ns: ns, shard: st.shard2.shardName}));
        ...
        multiShardSomeShardReads: {
            ...
            setUp: function(collName) {
                ...
                assert.commandWorked(
                    mongers.adminCommand({moveChunk: ns, find: {_id: 0}, to: st.shard1.shardName}));
                assert.commandWorked(
                    mongers.adminCommand({moveChunk: ns, find: {_id: 7}, to: st.shard2.shardName}));

                assert.eq(
                    0, mongers.getDB('config').chunks.count({ns: ns, shard: st.shard0.shardName}));
                assert.eq(
                    1, mongers.getDB('config').chunks.count({ns: ns, shard: st.shard1.shardName}));
                assert.eq(
                    1, mongers.getDB('config').chunks.count({ns: ns, shard: st.shard2.shardName}));
    ...
    function runTest(testScenario, {useCausalConsistency, commands, collName}) {
        ...
        for (let commandKey in commands) {
            ...
            const session =
                mainDb.getMonger().startSession({causalConsistency: useCausalConsistency});
