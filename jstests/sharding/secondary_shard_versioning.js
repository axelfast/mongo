...
(function() {
    ...
    // Use the mongers with the stale routing table to send read requests to the secondaries. 'local'
    ...
    // returns a stale shardVersion error, which provokes mongers to refresh its routing table and
    ...
    // mongers is fresh, because the recipient shard secondary was stale.
    profilerHasSingleMatchingEntryOrThrow({
        profileDB: donorShardSecondary.getDB('test'),
        filter: {
            "ns": "test.foo",
