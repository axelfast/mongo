...
struct CommandHelpers {
    // The type of the first field in 'cmdObj' must be monger::String. The first field is
    // interpreted as a collection name.
    static std::string parseNsFullyQualified(const BSONObj& cmdObj);

    // The type of the first field in 'cmdObj' must be monger::String or Symbol.
     ...
     * monger::String, then that field is interpreted as the collection name, and is
     * appended to 'dbname' after a '.' character. If the first field is not of type
     * monger::String, then 'dbname' is returned unmodified.
     */
     ...
     *    "wrapped" format historically used internally by mongers. Moving off of that format will be
     *    done as SERVER-29091.
     *
     * 2) Filter out generic arguments that shouldn't be blindly passed to the shards.  This is
     *    necessary because many mongers implementations of Command::run() just pass cmdObj through
     ...
     *
     * Ideally this function can be deleted once mongers run() implementations are more careful about
...
public:
     ...
     * session from the request in multi-staged invocations, like for example, mongers -> mongerd.
...
public:
     ...
     *
     * Note that this is never called on mongers. Sharded commands are responsible for forwarding
