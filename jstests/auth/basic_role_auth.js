...
var testOps = function(db, allowedActions) {
    ...
    (function() {
        var newConn = new Monger(db.getMonger().host);
    ...
    var isMongers = db.runCommand({isdbgrid: 1}).isdbgrid;
    // Note: fsyncUnlock is not supported in mongers.
    if (!isMongers) {
...
//   test: {function(Monger)} the test function to run which accepts a Monger connection
//                           object.
// }
var TESTS = [
    {
      ...
      test: function(conn) {
          ...
          var conn2 = new Monger(conn.host);
...

    {
      ...
      test: function(conn) {
          ...
          var newConn = new Monger(conn.host);
          ...
          var newConn3 = new Monger(conn.host);
 ...
 *
 * @param conn {Monger} a connection to a mongerd or mongers to test.
 */
