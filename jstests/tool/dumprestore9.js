...
if (0) {
    (function() {
        ...
        dumpdir = MongerRunner.dataDir + "/dumprestore9-dump1/";
        resetDbpath(dumpdir);
        var exitCode = MongerRunner.runMongerTool("mongerdump", {
            host: s.s0.host,
            out: dumpdir,
        });
        assert.eq(0, exitCode, "mongerdump failed to dump data through one of the mongers processes");
        ...
        s = new ShardingTest({name: "dumprestore9b", shards: 2, mongers: 3, other: {chunkSize: 1}});
        ...
        assert.eq(
            0, exitCode, "mongerrestore failed to restore data through the other mongers process");
