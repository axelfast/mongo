...
TEST(MockDBClientConnTest, InsertAndQuery) {
    ...
    {
        MockDBClientConnection conn(&server);
        std::unique_ptr<monger::DBClientCursor> cursor = conn.query(NamespaceString(ns));
        ASSERT(!cursor->more());
...
TEST(MockDBClientConnTest, QueryWithNoResults) {
    ...
    server.insert(ns, BSON("x" << 1));
    MockDBClientConnection conn(&server);
    std::unique_ptr<monger::DBClientCursor> cursor = conn.query(NamespaceString("other.ns"));
...
TEST(MockDBClientConnTest, MultiNSInsertAndQuery) {
    ...
    {
        MockDBClientConnection conn(&server);
        std::unique_ptr<monger::DBClientCursor> cursor = conn.query(NamespaceString(ns1));
    ...
    {
        MockDBClientConnection conn(&server);
        std::unique_ptr<monger::DBClientCursor> cursor = conn.query(NamespaceString(ns2));
    ...
    {
        MockDBClientConnection conn(&server);
        std::unique_ptr<monger::DBClientCursor> cursor = conn.query(NamespaceString(ns3));
...
TEST(MockDBClientConnTest, Shutdown) {
...

    {
        ...
        ASSERT_THROWS(conn.query(NamespaceString("test.user")), monger::NetworkException);
    }
...
TEST(MockDBClientConnTest, Restart) {
    ...
    server.shutdown();
    ASSERT_THROWS(conn1.query(NamespaceString("test.user")), monger::NetworkException);

    // New connections shouldn't work either
    MockDBClientConnection conn2(&server);
    ASSERT_THROWS(conn2.query(NamespaceString("test.user")), monger::NetworkException);
