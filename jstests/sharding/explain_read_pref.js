...
var assertCorrectTargeting = function(explain, isMongers, secExpected) {
    ...
    var explainDestConn = new Monger(serverInfo.host + ':' + serverInfo.port);
...
var testAllModes = function(conn, isMongers) {
    ...
    [
        ...
        assertCorrectTargeting(explain, isMongers, secExpected);

        // Set read pref on the connection.
        var oldReadPrefMode = testDB.getMonger().getReadPrefMode();
        var oldReadPrefTagSet = testDB.getMonger().getReadPrefTagSet();
        try {
            testDB.getMonger().setReadPref(mode, tagSets);
            ...
            testDB.getMonger().setReadPref(oldReadPrefMode, oldReadPrefTagSet);
        }
    });
};
...
// Force mongers to reconnect after our reconfig and also create the test database
