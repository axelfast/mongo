...
// Checking UUID consistency involves reading from the config server through mongers, but this test
// sets an invalid readPreference on the connection to the mongers.
 ...
 *
 * @param conn {Monger} the connection object of which to test the read
 *     preference functionality.
 * @param hostList {Array.<Monger>} list of the replica set host members.
 * @param isMongers {boolean} true if conn is a mongers connection.
 ...
 */
var testReadPreference = function(conn, hostList, isMongers, mode, tagSets, secExpected) {
    ...
    // Mongers doesn't implement geoSearch; test it only with ReplicaSetConnection.
    if (!isMongers) {
 ...
 * @param tagSets {Array.<Object>} list of tag sets to use
 */
var testBadMode = function(conn, hostList, isMongers, mode, tagSets) {
...
var testAllModes = function(conn, hostList, isMongers) {
    ...
    [
        ...
        setUp();
        testReadPreference(conn, hostList, isMongers, mode, tagSets, secExpected);
        tearDown();
    });
    ...
    [
        ...
        setUp();
        testBadMode(conn, hostList, isMongers, mode, tagSets);
        tearDown();
    });
};
...
// Force mongers to reconnect after our reconfig
assert.soon(function() {
    try {
        st.s.getDB('foo').runCommand({create: 'foo'});
        return true;
    } catch (x) {
        // Intentionally caused an error that forces mongers's monitor to refresh.
...
var replConn = new Monger(st.rs0.getURL());
...
jsTest.log('Starting test for mongers connection');
