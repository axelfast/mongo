...
(function() {
    ...
    // Obtain a connection to the mongerS and one direct connection to each shard.
    const shard0 = st.rs0.getPrimary();
    const shard1 = st.rs1.getPrimary();
    const mongers = st.s;

    const configDB = mongers.getDB("config");

    const mongersDB = mongers.getDB(jsTestName());
    ...
    function runSampleAndConfirmResults({sampleSize, comment, expectedPlanSummaries}) {
        // Run the aggregation via mongerS with the given 'comment' parameter.
        assert.eq(
            mongersColl.aggregate([{$sample: {size: sampleSize}}], {comment: comment}).itcount(),
            sampleSize);

        // Obtain the explain output for the aggregation.
        const explainOut =
            assert.commandWorked(mongersColl.explain().aggregate([{$sample: {size: sampleSize}}]));
    ...
    st.ensurePrimaryShard(mongersDB.getName(), shard0.name);

    // Shard the collection on {_id: 1}, split at {_id: 0} and move the empty upper chunk to shard1.
    st.shardColl(mongersColl.getName(), {_id: 1}, {_id: 0}, {_id: 0}, mongersDB.getName());

    // Write some documents to the lower chunk on shard0.
    for (let i = (-200); i < 0; ++i) {
        assert.commandWorked(mongersColl.insert({_id: i}));
    }
    ...
    // Confirm that there are 101 documents on shard1 and mongerS can see the 1 non-orphan.
    assert.eq(mongersColl.find({_id: {$gte: 0}}).itcount(), 1);
