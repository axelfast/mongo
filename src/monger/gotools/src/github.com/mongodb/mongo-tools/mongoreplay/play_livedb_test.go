...
const (
	...
	nonAuthTestServerURL = "mongerdb://localhost"
	authTestServerURL    = "mongerdb://authorizedUser:authorizedPwd@localhost"
	testDB               = "mongerreplay"
	testCollection       = "test"
...
func getPrimaryPort(session *mgo.Session) (string, error) {
	...
	isMongersTestServer = (res.Msg == "isdbgrid")
	if isMongersTestServer {
		return "", nil
	}
...
func TestMain(m *testing.M) {
	if !testtype.HasTestType(testtype.MongerReplayTestType) {
		os.Exit(0)
	}
...
// TestOpInsertLiveDB tests the functionality of mongerreplay replaying inserts
// against a live database Generates 20 recorded inserts and passes them to the
// main execution of mongerreplay and queries the database to verify they were
...
func TestOpInsertLiveDB(t *testing.T) {
	...
	// run mongerreplay's Play loop with the stubbed objects
	...
	for i := 0; i < numInserts; i++ {
		...
		// All commands should be inserts into mongerreplay.test
		if stat.OpType != "insert" ||
			stat.Ns != "mongerreplay.test" {
			t.Errorf("Expected to see an insert into mongerreplay.test, but instead saw %v, %v\n", stat.OpType, stat.Command)
		}
	}
...
// TestUpdateOpLiveDB tests the functionality of mongerreplay replaying an update
// against a live database Generates 20 recorded inserts and an update and
// passes them to the main execution of mongerreplay and queries the database to
...
func TestUpdateOpLiveDB(t *testing.T) {
	...
	if stat.OpType != "update" ||
		stat.Ns != "mongerreplay.test" {
		t.Errorf("Expected to see an update to mongerreplay.test, but instead saw %v, %v\n", stat.OpType, stat.Ns)
	}
	if err := teardownDB(); err != nil {
		t.Error(err)
	}
...
// TestQueryOpLiveDB tests the functionality of some basic queries through mongerreplay.
// It generates inserts and queries and sends them to the main execution of mongerreplay.
...
func TestQueryOpLiveDB(t *testing.T) {
	...
	// created to ensure that they match what we expected mongerreplay to have
	// executed
	for i := 0; i < numQueries; i++ {
...
// through mongerreplay. It generates inserts, a query, and a series of getmores
...
func TestOpGetMoreMultiCursorLiveDB(t *testing.T) {
	...
	go func() {
		...
		// be played in mongerreplay's main Play function
...
func TestOpKillCursorsLiveDB(t *testing.T) {
	...
	go func() {
		...
		// generate numInserts RecordedOp inserts and send them to a channel to be played in mongerreplay's main Play function
		t.Logf("Generating %v inserts\n", numInserts)
		err := generator.generateInsertHelper(insertName, 0, numInserts)
		if err != nil {
			t.Error(err)
	...
	for i := 0; i < 2; i++ {
		...
		if stat.OpType != "getmore" ||
			...
			t.Errorf("Getmore Not matched: Expected OpType: %s NumReturned: %d NameSpace: %s --- Found OpType: %s NumReturned: %d NameSpace: %s",
				"getmore", 2, "mongerreplay.test", stat.OpType, stat.NumReturned, stat.Ns)
		}
	}
	...
	for i := 0; i < 2; i++ {
		...
		if stat.OpType != "getmore" ||
			...
			stat.Ns != "mongerreplay.test" {
			t.Errorf("Getmore Not matched: Expected OpType: %s NumReturned: %d NameSpace: %s --- Found OpType: %s NumReturned: %d NameSpace: %s",
				"getmore", 0, "mongerreplay.test", stat.OpType, stat.NumReturned, stat.Ns)
		}
	}
...
func TestCommandOpInsertLiveDB(t *testing.T) {
	...
	if isMongersTestServer {
		t.Skipf("Skipping OpCommand test against mongers")
	}
	...
	for i := 0; i < numInserts; i++ {
		...
		if stat.OpType != "op_command" ||
			stat.Command != "insert" ||
			stat.Ns != "mongerreplay" {
			t.Errorf("Expected to see an insert into mongerreplay, but instead saw %v, %v into %v\n", stat.OpType, stat.Command, stat.Ns)
		}
	}
...
func TestCommandOpFindLiveDB(t *testing.T) {
	...
	// created, and ensure that they match what we expected mongerreplay to have
	// executed
	for i := 0; i < numFinds; i++ {
		stat := statRec.Buffer[numInserts+i]
		t.Logf("Stat result: %#v\n", stat)
...
// TestMsgOpInsertLiveDB tests the functionality of mongerreplay replaying inserts
// against a live database Generates 20 recorded inserts and passes them to the
// main execution of mongerreplay and queries the database to verify they were
// completed. It then checks its BufferedStatCollector to ensure the inserts
// match what we expected
...
func getmoreTestHelper(t *testing.T,
	...
	go func() {
		...
		// for use in mongerreplay's Play()
		...
		// generate a query with a known requestID to be played in mongerreplay
		...
		// information can be correlated by mongerreplay
