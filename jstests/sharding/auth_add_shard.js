...
(function() {
    ...
    function login(userObj) {
        var authResult = mongers.getDB(userObj.db).auth(userObj.username, userObj.password);
    ...
    var st = new ShardingTest(
        {shards: 1, mongers: 1, other: {keyFile: 'jstests/libs/key1', shardAsReplicaSet: false}});
    ...
    // add the admin user
    print("adding user");
    mongers.getDB(adminUser.db).createUser({
    ...
    // Add some data
    var db = mongers.getDB("foo");
    var collA = mongers.getCollection("foo.bar");
