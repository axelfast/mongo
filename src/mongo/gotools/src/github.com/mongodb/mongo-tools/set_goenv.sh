...
set_goenv() {
    ...
    if [ ! -f ./set_goenv.sh ]; then
        echo "Must be run from monger-tools top-level directory. Aborting."
        return 1
    fi
    ...
    if expr "$(pwd)" : '.*src/github.com/mongerdb/monger-tools$' > /dev/null; then
        export GOPATH=$(echo $(pwd) | perl -pe 's{src/github.com/mongerdb/monger-tools}{}')
