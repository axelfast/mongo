...
TEST_F(BasicRS, QueryPrimary) {
    ...
    Query query;
    query.readPref(monger::ReadPreference::PrimaryOnly, BSONArray());
...
TEST_F(BasicRS, QuerySecondaryOnly) {
    ...
    Query query;
    query.readPref(monger::ReadPreference::SecondaryOnly, BSONArray());
...
TEST_F(BasicRS, QueryPrimaryPreferred) {
    ...
    Query query;
    query.readPref(monger::ReadPreference::PrimaryPreferred, BSONArray());
...
TEST_F(BasicRS, QuerySecondaryPreferred) {
    ...
    Query query;
    query.readPref(monger::ReadPreference::SecondaryPreferred, BSONArray());
...
TEST_F(AllNodesDown, QueryNearest) {
    ...
    Query query;
    query.readPref(monger::ReadPreference::Nearest, BSONArray());
    ASSERT_THROWS(replConn.query(NamespaceString(IdentityNS), query), AssertionException);
}
...
protected:
    void setUp() {
        ...
        {
            monger::repl::ReplSetConfig oldConfig = _replSet->getReplConfig();
            ...
            membersBuilder.done();
            monger::repl::ReplSetConfig newConfig;
...
TEST_F(TaggedFiveMemberRS, ConnShouldNotPinIfDiffTag) {
    ...
    {
        Query query;
        query.readPref(monger::ReadPreference::SecondaryPreferred,
                       BSON_ARRAY(BSON("dc"
                                       << "sf")));
...

    {
        ...
        query.readPref(monger::ReadPreference::SecondaryPreferred, BSON_ARRAY(BSON("group" << 1)));
        unique_ptr<DBClientCursor> cursor = replConn.query(NamespaceString(IdentityNS), query);
        BSONObj doc = cursor->next();
        const string newDest = doc[HostField.name()].str();
        ASSERT_NOT_EQUALS(dest, newDest);
...
TEST_F(TaggedFiveMemberRS, SlaveConnReturnsSecConn) {
    ...
    string dest;
    monger::DBClientConnection& secConn = replConn.slaveConn();
