...
def _find_linter(linter, config_dict):
    ...
    # 3. The current path
    # 4. In '/opt/mongerdbtoolchain/v3/bin' if virtualenv is set up.
    ...
    if sys.platform == "win32":
        ...
        # '/opt/mongerdbtoolchain/v3/bin', but they may point to the wrong interpreter.
        cmd_str = os.path.join(python_dir, linter.cmd_name)
    ...
    # to use the linters installed in '/opt/mongerdbtoolchain/v3/bin'.
    cmd = [sys.executable, os.path.join('/opt/mongerdbtoolchain/v3/bin', linter.cmd_name)]
    if _check_version(linter, cmd, linter.get_lint_version_cmd_args()):
        return base.LinterInstance(linter, cmd)
