...
var st = new ShardingTest({shards: 2, mongers: 1, other: {shardAsReplicaSet: false}});

var mongers = st.s0;
var coll = mongers.getCollection("foo.bar");
var admin = mongers.getDB("admin");
var shards = mongers.getCollection("config.shards").find().toArray();
...
// Ensure that the doc count is correct and that the mongers query path can handle docs near the 16MB
