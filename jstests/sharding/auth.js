...
(function() {
    ...
    print("Restart mongers with different auth options");
    s.restartMongers(0);
    login(adminUser);
    ...
    // SERVER-4114 is triggered when 1 mongerd connects to another for final reduce it's not
    ...
    assert.eq(0, exitCode, "mongerdump failed to run with authentication enabled");

    // Test read only users
    print("starting read only tests");

    var readOnlyS = new Monger(s.getDB("test").getMonger().host);
    ...
    // fsyncUnlock doesn't work in mongers anyway, so no need check authorization for it
    /*
