...
(function() {
    ...
    var st = new ShardingTest({shards: 2, mongers: 4});
    ...
    // mongers0 still thinks that { x: 1 } belong to st.shard1.shardName, but should be able to
    ...
    // Set mongers2 & mongers3 to version 2|0|a
    ...
    // mongers versions: s0, s2, s3: 2|0|a
    ...
    // mongers0 still thinks { x: 10 } belong to st.shard0.shardName, but since coll is dropped,
    ...
    // mongers versions: s0: 0|0|0, s2, s3: 2|0|a
    ...
    // mongers2 still thinks that { x: 1 } belong to st.shard0.shardName, but should be able to
    // refresh it's metadata correctly.
    assert.neq(null, testDB_s2.user.findOne({x: 1}));
    ...
    // 4th mongers still thinks that { x: 1 } belong to st.shard0.shardName, but should be able to
    // refresh it's metadata correctly.
    assert.neq(null, testDB_s3.user.findOne({x: 1}));

    ///////////////////////////////////////////////////////
    // Test mongers thinks unsharded when it's actually sharded
    // mongers current versions: s0: 0|0|0, s2, s3: 2|0|b

    // Set mongers0 to version 0|0|0
    testDB_s0.user.drop();
    ...
    // Needs to also set mongers1 to version 0|0|0, otherwise it'll complain that collection is
    // already sharded.
    ...
    // 1st mongers thinks that collection is unshareded and will attempt to query primary shard.
    assert.neq(null, testDB_s0.user.findOne({x: 1}));
