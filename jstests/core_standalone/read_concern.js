// This test verifies readConcern behavior on a standalone mongerd or embedded
...
(function() {
    ...
    assert.commandWorked(t.runCommand({find: "read_concern", readConcern: {level: "local"}}),
                         "expected local readConcern to succeed on standalone mongerd");

    // Available readConcern succeed.
    assert.commandWorked(t.runCommand({find: "read_concern", readConcern: {level: "available"}}),
                         "expected available readConcern to succeed on standalone mongerd");
    ...
    if (isWiredTiger(db) || (isEphemeral(db) && !isEphemeralForTest(db))) {
        ...
        assert.commandWorked(majority_result,
                             "expected majority readConcern to succeed on standalone mongerd");
    } else {
        // Majority readConcern fail.
        assert.commandFailedWithCode(
            majority_result,
            [ErrorCodes.ReadConcernMajorityNotEnabled, ErrorCodes.NotImplemented],
            "expected majority readConcern to fail on standalone mongerd");
    }
    ...
    assert.commandFailedWithCode(
        ...
        "expected snapshot readConcern to fail on standalone mongerd");
