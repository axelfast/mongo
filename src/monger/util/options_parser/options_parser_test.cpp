...
#define TEST_CONFIG_PATH(x) "src/monger/util/options_parser/test_config_files/" x
...
TEST(Parsing, DefaultValuesNotInBSON) {
    ...
    monger::BSONObj expected = BSON("val1" << 6);
...
TEST(YAMLConfigFile, OutputConfig) {
    ...
    OptionsParserTester parser;
    parser.setConfig("config.yaml",
                     "systemLog: { path: /tmp/mongerd.log }\n"
                     "command: [ monger, mongerd, mongers ]");
    ...
    ASSERT_EQ(env.toYAML(),
              ...
              "  scramSHAIterationCount: 12345\n"
              "systemLog:\n"
              "  path: /tmp/mongerd.log");
}

void TestFile(std::vector<unsigned char> contents, bool valid) {
    monger::unittest::TempDir tempdir("options_testpath");
