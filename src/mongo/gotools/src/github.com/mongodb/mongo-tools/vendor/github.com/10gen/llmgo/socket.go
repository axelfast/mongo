...
type MongerSocket struct {
	sync.Mutex
	server        *MongerServer // nil when cached
	Conn          net.Conn
...
func NewSocket(server *MongerServer, conn net.Conn, timeout time.Duration) *MongerSocket {
	socket := &MongerSocket{
		Conn:       conn,
		addr:       server.Addr,
		server:     server,
...
func NewDumbSocket(conn net.Conn) *MongerSocket {
	server := &MongerServer{}
	return &MongerSocket{
		server:     server,
		addr:       server.Addr,
...
func (socket *MongerSocket) Server() *MongerServer {
	socket.Lock()
...
func (socket *MongerSocket) ServerInfo() *mongerServerInfo {
	socket.Lock()
...
func (socket *MongerSocket) InitialAcquire(serverInfo *mongerServerInfo, timeout time.Duration) error {
	socket.Lock()
...
func (socket *MongerSocket) Acquire() (info *mongerServerInfo) {
	socket.Lock()
	if socket.references == 0 {
...
func (socket *MongerSocket) Release() {
	socket.Lock()
	if socket.references == 0 {
...
func (socket *MongerSocket) SetTimeout(d time.Duration) {
	socket.Lock()
	socket.timeout = d
	socket.Unlock()
}
...
func (socket *MongerSocket) updateDeadline(which deadlineType) {
	var when time.Time
	if socket.timeout > 0 {
...
func (socket *MongerSocket) Close() {
	socket.kill(errors.New("Closed explicitly"), false)
}

func (socket *MongerSocket) kill(err error, abend bool) {
	socket.Lock()
	if socket.dead != nil {
...
func (socket *MongerSocket) SimpleQuery(op *QueryOp) (data []byte, replyOp *ReplyOp, err error) {
...
func (socket *MongerSocket) Query(ops ...interface{}) (err error) {
...
// document ever seen.
func (socket *MongerSocket) readLoop() {
