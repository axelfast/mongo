...
StatusWith<executor::TaskExecutor::EventHandle> BlockingResultsMerger::getNextEvent() {
    // If we abandoned a previous event due to a mongerS-side timeout, wait for it first.
    if (_leftoverEventFromLastTimeout) {
        ...
        // getMores (from the user to mongers), the response may have been an empty batch, and the
