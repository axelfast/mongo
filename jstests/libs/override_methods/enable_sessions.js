...
(function() {
    ...
    const getDBOriginal = Monger.prototype.getDB;
    ...
    function runCommandWithLsidCheck(conn, dbName, cmdName, cmdObj, func, makeFuncArgs) {
        ...
        if (!cmdObjUnwrapped.hasOwnProperty("lsid")) {
            // TODO: SERVER-30848 fixes getMore requests to use a session in the monger shell.
    ...
    Monger.prototype.getDB = function(dbName) {
        if (jsTest.options().disableEnableSessions) {
    ...
    db = db.getMonger().getDB(db.getName());
