...
(function() {
    ...
    const mongersDB = st.s0.getDB("merge_write_concern");
    ...
    function testWriteConcernError(rs) {
        ...
        withEachMergeMode(({whenMatchedMode, whenNotMatchedMode}) => {
            const res = mongersDB.runCommand({
                aggregate: "source",
                pipeline: [{
                    $merge: {
                        into: "target",
    ...
    st.shardColl(source, {_id: 1}, {_id: 0}, {_id: 1}, mongersDB.getName());
    testWriteConcernError(shard0);
    ...
    st.shardColl(target, {_id: 1}, {_id: 10}, {_id: 10}, mongersDB.getName());
