...
# This script used to create simulators called 'monger-sim', but then
...
xcrun simctl list | grep monger-sim | awk '{print $1}' | xargs xcrun simctl delete || true
...
_SimId=$(xcrun simctl create mongerdb-simulator-$DEVICE.$RUNTIME "com.apple.CoreSimulator.SimDeviceType.$DEVICE" "com.apple.CoreSimulator.SimRuntime.$RUNTIME")
