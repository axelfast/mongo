...
sh._lastMigration = function(ns) {
    ...
    if (!ns) {
        ...
        if (ns.indexOf(".") > 0) {
            config = db.getSisterDB("config");
            coll = db.getMonger().getCollection(ns);
        } else {
            config = db.getSisterDB("config");
...
function printShardingStatus(configDB, verbose) {
    ...
    var mostRecentMongers = configDB.mongers.find().sort({ping: -1}).limit(1);
    var mostRecentMongersTime = null;
    var mongersAdjective = "most recently active";
    if (mostRecentMongers.hasNext()) {
        mostRecentMongersTime = mostRecentMongers.next().ping;
        // Mongerses older than the threshold are the most recent, but cannot be
        // considered "active" mongerses. (This is more likely to be an old(er)
        // configdb dump, or all the mongerses have been stopped.)
        if (mostRecentMongersTime.getTime() >= Date.now() - mongersActiveThresholdMs) {
            mongersAdjective = "active";
        }
    }

    output(1, mongersAdjective + " mongerses:");
    if (mostRecentMongersTime === null) {
        output(2, "none");
    } else {
        var recentMongersQuery = {
            ping: {
                $gt: (function() {
                    var d = mostRecentMongersTime;
        ...
        if (verbose) {
            configDB.mongers.find(recentMongersQuery).sort({ping: -1}).forEach(function(z) {
                output(2, tojsononeline(z));
            });
        } else {
            configDB.mongers
                .aggregate([
                    {$match: recentMongersQuery},
