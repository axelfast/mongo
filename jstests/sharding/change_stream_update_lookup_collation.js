...
(function() {
    ...
    assert.commandWorked(
        mongersDB.runCommand({create: mongersColl.getName(), collation: caseInsensitive}));
    ...
    assert.commandWorked(
        mongersDB.adminCommand({split: mongersColl.getFullName(), middle: {shardKey: "aBC"}}));

    // Move the [MinKey, 'aBC') chunk to st.shard1.shardName.
    assert.commandWorked(mongersDB.adminCommand(
        {moveChunk: mongersColl.getFullName(), find: {shardKey: "ABC"}, to: st.rs1.getURL()}));
    ...
    assert.writeOK(mongersColl.insert({_id: "abc_1", shardKey: "ABC"}));
    assert.writeOK(mongersColl.insert({_id: "abc_2", shardKey: "ABC"}));
    assert.writeOK(mongersColl.insert({_id: "abc_1", shardKey: "abc"}));
    assert.writeOK(mongersColl.insert({_id: "abc_2", shardKey: "abc"}));
    ...
    // clusterTime sent from mongers will ensure that each corresponding oplog entry has a distinct
    // timestamp and so will appear in the change stream in the order we expect.
    let updateResult = mongersColl.updateOne({shardKey: "abc", _id: "abc_1"},
                                            {$set: {updatedCount: 1}},
                                            {collation: {locale: "simple"}});
    assert.eq(1, updateResult.modifiedCount);
    updateResult = mongersColl.updateOne({shardKey: "ABC", _id: "abc_1"},
                                        {$set: {updatedCount: 1}},
                                        {collation: {locale: "simple"}});
    assert.eq(1, updateResult.modifiedCount);

    function numIdIndexUsages(host) {
        return host.getCollection(mongersColl.getFullName())
    ...
    assert.writeOK(mongersColl.insert({_id: "abç_1", shardKey: "ABÇ"}));
    assert.writeOK(mongersColl.insert({_id: "abç_2", shardKey: "ABÇ"}));
    assert.writeOK(mongersColl.insert({_id: "abç_1", shardKey: "abç"}));
    assert.writeOK(mongersColl.insert({_id: "abç_2", shardKey: "abç"}));

    assert.eq(mongersColl.find({shardKey: "abc"}).collation(strengthOneCollation).itcount(), 8);

    const strengthOneChangeStream = mongersColl.aggregate(
        [
