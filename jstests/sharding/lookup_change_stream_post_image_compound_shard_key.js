...
(function() {
    ...
    assert.commandWorked(mongersDB.adminCommand(
        {shardCollection: mongersColl.getFullName(), key: {a: 1, b: 1, c: 1}}));
    ...
    assert.commandWorked(mongersDB.adminCommand(
        {split: mongersColl.getFullName(), middle: {a: 1, b: MinKey, c: MinKey}}));
    ...
    const changeStreamSingleColl = mongersColl.watch([], {fullDocument: "updateLookup"});
    const changeStreamWholeDb = mongersDB.watch([], {fullDocument: "updateLookup"});
    ...
    for (let id = 0; id < nDocs; ++id) {
        ...
        assert.writeOK(mongersColl.insert(documentKey));
        assert.writeOK(mongersColl.update(documentKey, {$set: {updatedCount: 1}}));
    }
    ...
    for (let id = 0; id < nDocs; ++id) {
        const documentKey = Object.merge({_id: id}, shardKeyFromId(id));
        assert.writeOK(mongersColl.update(documentKey, {$set: {updatedCount: 2}}));
    }
