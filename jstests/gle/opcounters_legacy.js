...
var lastDB = db;
var monger = new Monger(db.getMonger().host);
monger.writeMode = function() {
    return "legacy";
};
db = monger.getDB(db.toString());
...
// 1. Insert.
//
// - mongerd, single insert:
//     counted as 1 op if successful, else 0
// - mongerd, bulk insert of N with continueOnError=true:
//     counted as K ops, where K is number of docs successfully inserted
// - mongerd, bulk insert of N with continueOnError=false:
//     counted as K ops, where K is number of docs successfully inserted
//
// - mongers
...
assert(db.getLastError());
assert.eq(opCounters.insert + (isMongers ? 2 : 3), db.serverStatus().opcounters.insert);
...
// 4. Query.
//
// - mongerd: counted as 1 op, regardless of errors
// - mongers: counted as 1 op if successful, else 0
//
...
assert.eq(opCounters.query + (isMongers ? 0 : 1), db.serverStatus().opcounters.query);
