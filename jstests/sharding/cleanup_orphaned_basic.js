...
(function() {
    ...
    var coll = mongers.getCollection(ns);

    // cleanupOrphaned fails against mongers ('no such command'): it must be run
    // on mongerd.
    assert.commandFailed(mongersAdmin.runCommand({cleanupOrphaned: ns}));
    ...
    assert.commandWorked(mongersAdmin.runCommand({enableSharding: coll.getDB().getName()}));

    st.ensurePrimaryShard(coll.getDB().getName(), st.shard0.shardName);

    assert.commandWorked(mongersAdmin.runCommand({shardCollection: ns, key: {_id: 1}}));
    ...
    // may fail.
    assert.commandWorked(mongersAdmin.runCommand({
    ...
    var coll2 = mongers.getCollection('foo.baz');

    assert.commandWorked(
        mongersAdmin.runCommand({shardCollection: coll2.getFullName(), key: {a: 1, b: 1}}));
