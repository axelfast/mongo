/**
 * Test that verifies FTDC works in mongers.
...
(function() {
    'use strict';
    let testPath1 = MongerRunner.toRealPath('ftdc_setdir1');
    let testPath2 = MongerRunner.toRealPath('ftdc_setdir2');
    let testPath3 = MongerRunner.toRealPath('ftdc_setdir3');
    // SERVER-30394: Use a directory relative to the current working directory.
    let testPath4 = 'ftdc_setdir4/';
    let testLog3 = testPath3 + "mongers_ftdc.log";
    let testLog4 = testPath4 + "mongers_ftdc.log";

    // Make the log file directory for mongers.
    mkdir(testPath3);
    mkdir(testPath4);

    // Startup 3 mongers:
    // 1. Normal MongerS with no log file to verify FTDC can be startup at runtime with a path.
    // 2. MongerS with explict diagnosticDataCollectionDirectoryPath setParameter at startup.
    // 3. MongerS with log file to verify automatic FTDC path computation works.
    ...
    function normpath(path) {
        // On Windows, strip the drive path because MongerRunner.toRealPath() returns a Unix Path
    ...
    function verifyFTDCStartsWithLogFile() {
        ...
        assert.eq(normpath(computedPath), normpath(testPath3 + "mongers_ftdc.diagnostic.data"));
