...
// RecordCommand stores settings for the mongerreplay 'record' subcommand
...
func Record(ctx *packetHandlerContext,
	...
	go func() {
		defer close(ch)
		var fail error
		for op := range ctx.mongerOpStream.Ops {
			// since we don't currently have a way to shutdown packetHandler.Handle()
			// continue to read from ctx.mongerOpStream.Ops even after a fatal error
			if fail != nil {
	...
	if err := ctx.packetHandler.Handle(ctx.mongerOpStream, -1); err != nil {
