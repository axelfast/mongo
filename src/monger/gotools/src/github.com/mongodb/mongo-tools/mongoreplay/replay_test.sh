...
-a, --assert JS-BOOL     condition for assertion after workload (used with -w);
    ...
    --dbpath             path for mongerd, can be cleared by program
                         (defaults to $DBPATH)
...
-v, --verbose            unsilence mongerreplay and make it slightly loud
-w, --workload JS-FILE   monger shell workload script (used with -a);
...
if [ $? != 0 ]; then
  log "mongerreplay must be in PATH"
  exit 1
fi
...
mkdir $DBPATH
log "starting MONGOD"
mongerd --port=$PORT --dbpath=$DBPATH >/dev/null &
MONGOPID=$!
...
check_integrity() {
  ...
  do
    ...
    monger --port=$PORT --quiet mongerreplay_test --eval "assert($assertion)" >/dev/null
    STATUS=$?
    if [ $STATUS != 0 ]; then
...
sleep 1
monger --port=$PORT mongerreplay_test --eval "db.dropDatabase()" >/dev/null 2>&1
...
do
  script="${WORKLOADS[$i]}"
  monger --port=$PORT --quiet mongerreplay_test "$script" >/dev/null &
  WorkerPIDs+=("$!")
done
# join
...
# clean up database
monger --port=$PORT mongerreplay_test --eval "db.dropDatabase()" >/dev/null 2>&1
sleep 1 # mongerreplay play should certainly happen after the drop

log "running mongerreplay RECORD"
mongerreplay record $SILENT $VERBOSE $DEBUG -f=tmp.pcap -p=tmp.playback >/dev/null
TAPECODE=$?
if [ "$TAPECODE" != 0 ]; then
  log "mongerreplay failed with code $TAPECODE"
  exit 1
fi

log # newline to separate replay

log "starting mongerreplay PLAY"
REPLAY_START=`date`
sleep 1
mongerreplay play $SILENT $VERBOSE $DEBUG --host "localhost:$PORT" -p=tmp.playback --report tmp.report >/dev/null
sleep 1
REPLAY_END=`date`
log "finished mongerreplay PLAY"
