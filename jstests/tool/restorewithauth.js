/* SERVER-4972
 * Test for mongerrestore on server with --auth allows restore without credentials of colls
 * with no index
 */
/*
 * 1) Start monger without auth.
 * 2) Write to collection
 * 3) Take dump of the collection using mongerdump.
 * 4) Drop the collection.
 * 5) Stop mongerd from step 1.
 * 6) Restart mongerd with auth.
...
var conn = MongerRunner.runMongerd({nojournal: "", bind_ip: "127.0.0.1"});
...
// get data dump
var dumpdir = MongerRunner.dataDir + "/restorewithauth-dump1/";
resetDbpath(dumpdir);
...
assert.eq(0, exitCode, "mongerdump failed to dump data from mongerd without auth enabled");
...
// start mongerd with --auth
conn = MongerRunner.runMongerd({auth: "", nojournal: "", bind_ip: "127.0.0.1"});
...
assert.neq(0,
           exitCode,
           "mongerrestore shouldn't have been able to restore data to mongerd with auth enabled");
...
assert.eq(0, exitCode, "mongerrestore failed to restore data to mongerd with auth enabled");
...
assert.eq(0, exitCode, "mongerrestore failed to restore the 'foo' database");
