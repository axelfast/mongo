 ...
 * WT-2696 and WT-2706.  Have several threads inserting unique data.  Kill -9 mongerd.  After
...
(function() {
    ...
    var dbpath = MongerRunner.dataPath + 'wt_unclean_shutdown';
    resetDbpath(dbpath);
    ...
    var insertWorkload = function(host, start, end) {
        ...
        for (var i = start; i < end; i++) {
            ...
            try {
                ...
                // Terminate the loop when mongerd is killed.
                break;
            }
        }
        // Return i, the last record we were trying to insert.  It is possible that mongerd gets
    ...
    // Sleep for sometime less than a minute so that mongerd has not yet written a checkpoint.
    ...
    // Mongerd needs an unclean shutdown so that WT recovery is forced on restart and we can detect
    // any missing records.
    ...
    for (var i = 0; i < retData.length; i++) {
        ...
        for (var j = thread_data.start; j <= thread_data.end; j++) {
            ...
            // of the range are missing.  Records at the end may be missing due to when mongerd
