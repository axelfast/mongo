...
(function() {
    ...
    function runTest(collName, shardKey) {
        ...
        assert.commandWorked(st.s0.adminCommand({enableSharding: mongersDB.getName()}));
        ...
        assertErrorCode(mongersDB.NegativeTest,
        ...
        assertErrorCode(
            mongersColl, [{$changeStream: {}}, {$out: "shouldntWork"}], ErrorCodes.IllegalOperation);
        ...
        st.shardColl(mongersColl,
                     makeShardKey(1) /* shard key */,
                     makeShardKey(0) /* split at */,
                     makeShardKey(1) /* move to shard 1 */);

        // Write a document to each chunk.
        assert.writeOK(mongersColl.insert(makeShardKeyDocument(-1)));
        assert.writeOK(mongersColl.insert(makeShardKeyDocument(1)));

        let changeStream = mongersColl.aggregate([{$changeStream: {}}]);

        // Test that a change stream can see inserts on shard 0.
        assert.writeOK(mongersColl.insert(makeShardKeyDocument(1000)));
        assert.writeOK(mongersColl.insert(makeShardKeyDocument(-1000)));
        ...
        assert.writeOK(mongersColl.insert(makeShardKeyDocument(1001)));
        ...
        assert.writeOK(mongersColl.insert(makeShardKeyDocument(10, {a: 0, b: 0})));
        assert.writeOK(mongersColl.insert(makeShardKeyDocument(-10, {a: 0, b: 0})));
        changeStream = mongersColl.aggregate([{$changeStream: {}}]);

        assert.writeOK(mongersColl.update({a: 0}, {$set: {b: 2}}, {multi: true}));
        ...
        // 'internalQueryProhibitMergingOnMongers' parameter is set.
        assert.commandWorked(
            st.s0.adminCommand({setParameter: 1, internalQueryProhibitMergingOnMongerS: true}));
        let tempCursor = assert.doesNotThrow(() => mongersColl.aggregate([{$changeStream: {}}]));
        tempCursor.close();
        assert.commandWorked(
            st.s0.adminCommand({setParameter: 1, internalQueryProhibitMergingOnMongerS: false}));

        assert.writeOK(mongersColl.remove({}));
        ...
        // node.
        assert.writeOK(
            mongersColl.insert(makeShardKeyDocument(0, {a: 1}), {writeConcern: {w: "majority"}}));
        ...
        assert.soon(() => !TwoPhaseDropCollectionTest.collectionIsPendingDropInDatabase(
                        mongersColl.getDB(), mongersColl.getName()));
        assert.soon(() => changeStream.hasNext());
        ...
        assert.writeOK(
            mongersColl.insert(makeShardKeyDocument(-1), {writeConcern: {w: "majority"}}));
        assert.writeOK(mongersColl.insert(makeShardKeyDocument(1), {writeConcern: {w: "majority"}}));
        ...
        assert.writeOK(
            mongersColl.insert(makeShardKeyDocument(-2), {writeConcern: {w: "majority"}}));
        assert.writeOK(mongersColl.insert(makeShardKeyDocument(2), {writeConcern: {w: "majority"}}));
        ...
        changeStream =
            mongersColl.watch([], {resumeAfter: resumeToken, collation: {locale: "simple"}});
