...
// `monger::clonable_ptr`.  The basic use cases satisfied by each class are described in each class's
// documentation.
...
// is an attendant specialization of the `monger::clonable_traits` metafunction to provide the clone
...
namespace monger {
// This specialization of the `monger::clonable_traits` metafunction provides a model of a stateless
...
#ifndef _MSC_VER
    {
        monger::clonable_ptr<ClonableTest> p;

        p = std::make_unique<ClonableTest>();

        monger::clonable_ptr<ClonableTest> p2 = p;

        ASSERT_TRUE(p != p2);

        static_assert(sizeof(p) == sizeof(ClonableTest*),
                      "`monger::clonable_ptr< T >` should be `sizeof` a pointer when there is no "
                      "CloneFactory");
    }
#endif

    {
        monger::clonable_ptr<AltClonableTest> p;

        p = std::make_unique<AltClonableTest>();

        monger::clonable_ptr<AltClonableTest> p2 = p;

        ASSERT_TRUE(p != p2);
    }

    {
        monger::clonable_ptr<Alt2ClonableTest> p;

        p = std::make_unique<Alt2ClonableTest>();

        monger::clonable_ptr<Alt2ClonableTest> p2 = p;

        ASSERT_TRUE(p != p2);
    }

    {
        monger::clonable_ptr<FunctorClonable, FunctorClonable::CloningFunctionType> p{
            FunctorClonable::getCloningFunction()};

        p = std::make_unique<FunctorClonable>();

        monger::clonable_ptr<FunctorClonable, FunctorClonable::CloningFunctionType> p2 = p;

        ASSERT_TRUE(p != p2);

        monger::clonable_ptr<FunctorClonable, FunctorClonable::CloningFunctionType> p3{
            FunctorClonable::getCloningFunction()};
...
// `monger::clonable_ptr<Clonable>` are valid.  These tests assert nothing but provide a single
...
void construction() {
    ...
    { monger::clonable_ptr<Clonable>{}; }

    // Test construction from a nullptr
    { monger::clonable_ptr<Clonable>{nullptr}; }

    // Test construction from a Clonable pointer.
    {
        Clonable* const local = nullptr;
        monger::clonable_ptr<Clonable>{local};
    }

    // Test move construction.
    { std::ignore = monger::clonable_ptr<Clonable>{monger::clonable_ptr<Clonable>{}}; }

    // Test copy construction.
    {
        monger::clonable_ptr<Clonable> a;
        monger::clonable_ptr<Clonable> b{a};
    }

    // Test move assignment.
    {
        monger::clonable_ptr<Clonable> a;
        a = monger::clonable_ptr<Clonable>{};
    }

    // Test copy assignment.
    {
        monger::clonable_ptr<Clonable> a;
        monger::clonable_ptr<Clonable> b;
        b = a;
    }

    // Test unique pointer construction
    { monger::clonable_ptr<Clonable>{std::make_unique<Clonable>()}; }

    // Test unique pointer construction (conversion)
    {
        auto acceptor = [](const monger::clonable_ptr<Clonable>&) {};
        acceptor(std::make_unique<Clonable>());
    }

    // Test non-conversion pointer construction
    { static_assert(!std::is_convertible<Clonable*, monger::clonable_ptr<Clonable>>::value, ""); }

    // Test conversion unique pointer construction
    {
        static_assert(
            std::is_convertible<std::unique_ptr<Clonable>, monger::clonable_ptr<Clonable>>::value,
            "");
    }
}
...
void augmentedConstruction() {
    ...
    {
        static_assert(
            !std::is_default_constructible<monger::clonable_ptr<Clonable, CloneFactory>>::value, "");
    }

    // Test Clone Factory construction
    { monger::clonable_ptr<Clonable, CloneFactory>{Clonable::getCloningFunction()}; }

// TODO: Revist this when MSVC's enable-if and deletion on ctors works.
#ifndef _MSC_VER
    // Test non-construction from a nullptr
    {
        static_assert(!std::is_constructible<monger::clonable_ptr<Clonable, CloneFactory>,
                                             std::nullptr_t>::value,
                      "");
    }
#endif
    ...
    { monger::clonable_ptr<Clonable, CloneFactory>{nullptr, Clonable::getCloningFunction()}; }
...
// TODO: Revist this when MSVC's enable-if and deletion on ctors works.
    ...
    {
        static_assert(
            !std::is_constructible<monger::clonable_ptr<Clonable, CloneFactory>, Clonable*>::value,
            "");
    }
#endif
    ...
    {
        Clonable* const local = nullptr;
        monger::clonable_ptr<Clonable, CloneFactory> p{Clonable::getCloningFunction()};
        p.reset(local);
    }

    // Test move construction.
    {
        std::ignore = monger::clonable_ptr<Clonable, CloneFactory>{
            monger::clonable_ptr<Clonable, CloneFactory>{Clonable::getCloningFunction()}};
    }

    // Test copy construction.
    {
        monger::clonable_ptr<Clonable, CloneFactory> a{Clonable::getCloningFunction()};
        monger::clonable_ptr<Clonable, CloneFactory> b{a};
    }

    // Test augmented copy construction.
    {
        monger::clonable_ptr<Clonable, CloneFactory> a{Clonable::getCloningFunction()};
        monger::clonable_ptr<Clonable, CloneFactory> b{a, Clonable::getCloningFunction()};
    }
    ...
    {
        monger::clonable_ptr<Clonable, CloneFactory> a{Clonable::getCloningFunction()};
        a = monger::clonable_ptr<Clonable, CloneFactory>{Clonable::getCloningFunction()};
    }

    // Test copy assignment.
    {
        monger::clonable_ptr<Clonable, CloneFactory> a{Clonable::getCloningFunction()};
        monger::clonable_ptr<Clonable, CloneFactory> b{Clonable::getCloningFunction()};
        b = a;
    }

    // Test unique pointer construction
    {
        monger::clonable_ptr<Clonable, CloneFactory>{std::make_unique<Clonable>(),
    ...
    {
        static_assert(
            !std::is_convertible<monger::clonable_ptr<Clonable, CloneFactory>, Clonable*>::value,
            "");
    }
    ...
    {
        static_assert(
            !std::is_convertible<monger::clonable_ptr<Clonable, CloneFactory>, CloneFactory>::value,
            "");
    }
    ...
    // Test conversion unique pointer construction
    {
        static_assert(!std::is_convertible<std::unique_ptr<Clonable>,
                                           monger::clonable_ptr<Clonable, CloneFactory>>::value,
                      "");
    }
}
...
TEST(ClonablePtrTest, basic_construction_test) {
    // Do not default construct the object
    {
        ...
        monger::clonable_ptr<DetectDestruction> p;
        ASSERT_EQ(DetectDestruction::activeCount, 0);
    }
    ...
    {
        DestructionGuard check;
        monger::clonable_ptr<DetectDestruction> p{new DetectDestruction};
    ...
    {
        DestructionGuard check;
        monger::clonable_ptr<DetectDestruction> p{std::make_unique<DetectDestruction>()};
        ASSERT_EQ(DetectDestruction::activeCount, 1);
    }
    {
        DestructionGuard check;
    ...
    // Two separate constructions are unlinked
    {
        ...
        monger::clonable_ptr<DetectDestruction> p1{std::make_unique<DetectDestruction>()};
        ASSERT_EQ(DetectDestruction::activeCount, 1);

        {
            monger::clonable_ptr<DetectDestruction> p2{std::make_unique<DetectDestruction>()};
    ...
    // Two separate constructions can have opposite order and be unlinked
    {
        ...
        auto p1 = std::make_unique<monger::clonable_ptr<DetectDestruction>>(
            std::make_unique<DetectDestruction>());
        ASSERT_EQ(DetectDestruction::activeCount, 1);

        auto p2 = std::make_unique<monger::clonable_ptr<DetectDestruction>>(
            std::make_unique<DetectDestruction>());
        ASSERT_EQ(DetectDestruction::activeCount, 2);
...
TEST(ClonablePtrTest, basicEqualityTest) {
    ...
    monger::clonable_ptr<DetectDestruction> n1;
    monger::clonable_ptr<DetectDestruction> n2;
    monger::clonable_ptr<DetectDestruction> n3;

    monger::clonable_ptr<DetectDestruction> a = std::make_unique<DetectDestruction>();
    monger::clonable_ptr<DetectDestruction> b = std::make_unique<DetectDestruction>();
    monger::clonable_ptr<DetectDestruction> c = std::make_unique<DetectDestruction>();

    const monger::clonable_ptr<DetectDestruction>& ap = a;
    const monger::clonable_ptr<DetectDestruction>& bp = b;
    const monger::clonable_ptr<DetectDestruction>& cp = c;
    const monger::clonable_ptr<DetectDestruction>& ap2 = a;
    const monger::clonable_ptr<DetectDestruction>& bp2 = b;
    const monger::clonable_ptr<DetectDestruction>& cp2 = c;
...
TEST(ClonablePtrTest, ownershipStabilityTest) {
    {
        ...
        monger::clonable_ptr<DetectDestruction> cp = std::move(ptr_init);

        ASSERT(rp == cp.get());

        monger::clonable_ptr<DetectDestruction> cp2 = std::move(cp);

        ASSERT(rp == cp2.get());

        monger::clonable_ptr<DetectDestruction> cp3;
...

    {
        ...
        monger::clonable_ptr<DetectDestruction> cp{ptr_init.release()};
...
TEST(ClonablePtrTest, noObjectCopySemanticTest) {
    monger::clonable_ptr<ClonableObject> p;

    monger::clonable_ptr<ClonableObject> p2 = p;
    ASSERT(p == p2);

    monger::clonable_ptr<ClonableObject> p3;
...
TEST(ClonablePtrTest, objectCopySemanticTest) {
    monger::clonable_ptr<ClonableObject> p = std::make_unique<ClonableObject>(1);
    monger::clonable_ptr<ClonableObject> q = std::make_unique<ClonableObject>(2);
    ASSERT(p != q);
    ASSERT(*p != *q);
    ...
    monger::clonable_ptr<ClonableObject> q2 = q;
    ASSERT(q2 != q);
    ASSERT(q2 != p);
    ASSERT(q2 != p2);
    ASSERT(*q2 == *q);
...
TEST(ClonablePtrSimpleTest, simpleUsageExample) {
    monger::clonable_ptr<Interface> source;
    monger::clonable_ptr<Interface> sink;

    monger::clonable_ptr<Interface> instance = std::make_unique<StorageImplementation>();
