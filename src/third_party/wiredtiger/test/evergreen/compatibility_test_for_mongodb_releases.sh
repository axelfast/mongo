...
get_release()
{
	echo "$(git tag | grep "^mongerdb-$1.[0-9]" | sort -V | sed -e '$p' -e d)"
}
...
#
# arg1: MongerDB tagged release number or develop branch identifier.
#############################################################
build_rel()
{
	echo "=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-="
	...
	case "$1" in
        # Please note 'develop' here is planned as the future MongerDB release 4.2 - the only release that supports
