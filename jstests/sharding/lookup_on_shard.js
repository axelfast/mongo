// Test that a pipeline with a $lookup stage on a sharded foreign collection may be run on a mongerd.
(function() {

    load("jstests/noPassthrough/libs/server_parameter_helpers.js");  // For setParameterOnAllHosts.
    load("jstests/libs/discover_topology.js");                       // For findDataBearingNodes.

    const sharded = new ShardingTest({mongers: 1, shards: 2});
    ...
    const runTest = function() {
        (function testSingleLookupFromShard() {
            ...
            // the sharded collection) to be run on a mongerd.
            pipeline = [
                {
                  $lookup: {
        ...
        (function testMultipleLookupsFromShard() {
            // Run two lookups in a row (both on mongerd).
            pipeline = [
                {
                  $lookup: {
                      localField: "foreignId",
        ...
        (function testUnshardedLookupWithinShardedLookup() {
            ...
            for (let i = 0; i < results.length; i++) {
                ...
                for (let j = 0; j < nDocsForeignColl; j++) {
                    ...
                    // TODO SERVER-39016: Once a mongerd is able to target the primary shard when
