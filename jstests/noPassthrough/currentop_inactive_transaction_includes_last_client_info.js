...
(function() {
    ...
    assert.eq(currentOpEntry.appName, "MongerDB Shell");
    assert.eq(currentOpEntry.clientMetadata.application.name, "MongerDB Shell");
    assert.eq(currentOpEntry.clientMetadata.driver.name, "MongerDB Internal Client");

    // Create a new Client and run another operation on the same session.
    const otherClient = new Monger(rst.getPrimary().host);
