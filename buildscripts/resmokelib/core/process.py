...
class Process(object):
    ...
    def stop(self, kill=False):  # pylint: disable=too-many-branches
        ...
        if sys.platform == "win32":
            ...
            # Attempt to cleanly shutdown mongerd.
            if not kill and self.args and self.args[0].find("mongerd") != -1:
                monger_signal_handle = None
                try:
                    ...
                    if not monger_signal_handle:
                        ...
                        return
                    win32event.SetEvent(monger_signal_handle)
                    ...
                    win32api.CloseHandle(monger_signal_handle)
