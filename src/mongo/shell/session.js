 ...
 * https://github.com/mongerdb/specifications/blob/master/source/sessions/driver-sessions.rst#abstract
 */
var {
    ...
    function SessionOptions(rawOptions = {}) {
        ...
        // If the user specified --retryWrites to the monger shell, then we enable retryable
    ...
    function SessionAwareClient(client) {
         ...
         *
         * https://github.com/mongerdb/specifications/blob/5b53e0baca18ba111364d479a37fa9195ef801a6/
        ...
        function runClientFunctionWithRetries(
            ...
            // TODO SERVER-33921: Revisit how the monger shell decides whether it should retry a
            // command or not.
            const sessionOptions = driverSession.getOptions();
            let numRetries =
                ...
                if (numRetries > 0) {
                    ...
                    if (isRetryableCode(res.code)) {
                        ...
                        if (client.isReplicaSetConnection()) {
                            client._markNodeAsFailed(res._monger.host, res.code, res.errmsg);
                        }
                        continue;
                    }
                        ...
                        if (writeError !== undefined) {
                            ...
                            if (client.isReplicaSetConnection()) {
                                client._markNodeAsFailed(
                                    res._monger.host, writeError.code, writeError.errmsg);
                            }
                            continue;
                        }
                    }
                    ...
                    if (res.hasOwnProperty("writeConcernError") &&
                        ...
                        if (client.isReplicaSetConnection()) {
                            client._markNodeAsFailed(res._monger.host,
                                                     res.writeConcernError.code,
