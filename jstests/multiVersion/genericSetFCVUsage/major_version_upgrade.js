...
(function() {
    ...
    for (let i = 0; i < versions.length; i++) {
        ...
        assert.neq(
            null, conn, 'mongerd was unable to start up with options: ' + tojson(mongerdOptions));
        ...
        assert.eq(
            ...
            `monger should have inserted 1 document into collection ${version.testCollection}; ` +
                'options: ' + tojson(mongerdOptions));
    ...
    for (let i = 0; i < versions.length; i++) {
        ...
        assert.eq(
            2,
            testDB[version.testCollection].count(),
            `monger should have inserted 2 documents into collection ${version.testCollection}; ` +
                'nodes: ' + tojson(nodes));
