...
using monger::SafeNum;
using monger::Decimal128;

TEST(Basics, Initialization) {
    const SafeNum numInt(0);
    ASSERT_EQUALS(numInt.type(), monger::NumberInt);

    const SafeNum numLong(static_cast<int64_t>(0));
    ASSERT_EQUALS(numLong.type(), monger::NumberLong);

    const SafeNum numDouble(0.0);
    ASSERT_EQUALS(numDouble.type(), monger::NumberDouble);

    const SafeNum numDecimal(Decimal128("1.0"));
    ASSERT_EQUALS(numDecimal.type(), monger::NumberDecimal);
}
...
TEST(Comparison, EOO) {
    ...
    ASSERT_EQUALS(safeNumA.type(), monger::EOO);
    ASSERT_EQUALS(safeNumB.type(), monger::EOO);
...
TEST(Addition, UpConvertion) {
    ...
    ASSERT_EQUALS((zeroInt32 + zeroInt64).type(), monger::NumberLong);
    ASSERT_EQUALS((zeroInt64 + zeroInt32).type(), monger::NumberLong);
    ASSERT_EQUALS((zeroInt32 + zeroDouble).type(), monger::NumberDouble);
    ASSERT_EQUALS((zeroInt64 + zeroDouble).type(), monger::NumberDouble);
    ...
    ASSERT_EQUALS(stillInt32.type(), monger::NumberInt);
    ASSERT_EQUALS(stillInt64.type(), monger::NumberLong);
    ASSERT_EQUALS(stillDouble.type(), monger::NumberDouble);

    const SafeNum zeroDecimal(Decimal128(0));
    ASSERT_EQUALS((zeroInt64 + zeroDecimal).type(), monger::NumberDecimal);
    ASSERT_EQUALS((zeroInt32 + zeroDecimal).type(), monger::NumberDecimal);
    ASSERT_EQUALS((zeroDouble + zeroDecimal).type(), monger::NumberDecimal);
    ASSERT_EQUALS((zeroDecimal + zeroInt32).type(), monger::NumberDecimal);
    ASSERT_EQUALS((zeroDecimal + zeroInt64).type(), monger::NumberDecimal);
    ASSERT_EQUALS((zeroDecimal + zeroDouble).type(), monger::NumberDecimal);

    const SafeNum stillDecimal(zeroDecimal + zeroDecimal);
    ASSERT_EQUALS(stillDecimal.type(), monger::NumberDecimal);
}

TEST(Addition, Overflow32to64) {
    const SafeNum maxInt32(std::numeric_limits<int32_t>::max());
    ASSERT_EQUALS(maxInt32.type(), monger::NumberInt);

    const SafeNum int32PlusOne(maxInt32 + 1);
    ASSERT_EQUALS(int32PlusOne.type(), monger::NumberLong);

    const SafeNum int32MinusOne(maxInt32 + -1);
    ASSERT_EQUALS(int32MinusOne.type(), monger::NumberInt);
...
TEST(Addition, Overflow64toDouble) {
    ...
    ASSERT_EQUALS(maxInt64.type(), monger::NumberLong);

    // We don't overflow int64 to double.
    const SafeNum int64PlusOne(maxInt64 + 1);
    ASSERT_EQUALS(int64PlusOne.type(), monger::EOO);

    const SafeNum int64MinusOne(maxInt64 + -1);
    ASSERT_EQUALS(int64MinusOne.type(), monger::NumberLong);

    const SafeNum doubleResult(std::numeric_limits<int64_t>::max() + static_cast<double>(1));
    ASSERT_EQUALS(doubleResult.type(), monger::NumberDouble);
...
TEST(Addition, Negative32to64) {
    ...
    ASSERT_EQUALS(minInt32.type(), monger::NumberInt);

    const SafeNum int32MinusOne(minInt32 + -1);
    ASSERT_EQUALS(int32MinusOne.type(), monger::NumberLong);

    const SafeNum int32PlusOne(minInt32 + 1);
    ASSERT_EQUALS(int32PlusOne.type(), monger::NumberInt);
...
TEST(Addition, Negative64toDouble) {
    ...
    ASSERT_EQUALS(minInt64.type(), monger::NumberLong);

    // We don't overflow int64 to double.
    const SafeNum int64MinusOne(minInt64 + -1);
    ASSERT_EQUALS(int64MinusOne.type(), monger::EOO);

    const SafeNum int64PlusOne(minInt64 + 1);
    ASSERT_EQUALS(int64PlusOne.type(), monger::NumberLong);
...
TEST(BitAnd, 32and32) {
    ...
    ASSERT_EQUALS(monger::NumberInt, result.type());
...
TEST(BitAnd, 64and64) {
    ...
    const SafeNum result = val1 & val2;
    ASSERT_EQUALS(monger::NumberLong, result.type());
    ASSERT_TRUE(expected.isIdentical(result));
}
...
TEST(BitAnd, MixedSize) {
    ...
    ASSERT_EQUALS(monger::NumberLong, result_s_b.type());
    ASSERT_TRUE(expected.isIdentical(result_s_b));

    ASSERT_EQUALS(monger::NumberLong, result_b_s.type());
...
TEST(Multiplication, UpConvertion) {
    ...
    ASSERT_EQUALS((zeroInt32 * zeroDouble).type(), monger::NumberDouble);
    ASSERT_EQUALS((zeroInt64 * zeroDouble).type(), monger::NumberDouble);
    ASSERT_EQUALS((zeroDouble * zeroInt32).type(), monger::NumberDouble);
    ASSERT_EQUALS((zeroDouble * zeroInt64).type(), monger::NumberDouble);
...
TEST(Multiplication, Overflow32to64) {
    ...
    ASSERT_EQUALS(int32TimesOne.type(), monger::NumberInt);

    const SafeNum int32TimesTwo(maxInt32 * 2);
    ASSERT_EQUALS(int32TimesTwo.type(), monger::NumberLong);
}
...
TEST(Multiplication, Overflow64toDouble) {
    ...
    ASSERT_EQUALS(int64TimesTwo.type(), monger::EOO);
...
TEST(Multiplication, Negative64toDouble) {
    ...
    ASSERT_EQUALS(int64TimesOne.type(), monger::NumberLong);
...
TEST(Multiplication, 64OverflowsFourWays) {
    const SafeNum maxInt64(std::numeric_limits<int64_t>::max());
    const SafeNum minInt64(std::numeric_limits<int64_t>::min());
    ASSERT_EQUALS(monger::EOO, (maxInt64 * maxInt64).type());
    ASSERT_EQUALS(monger::EOO, (maxInt64 * minInt64).type());
    ASSERT_EQUALS(monger::EOO, (minInt64 * maxInt64).type());
    ASSERT_EQUALS(monger::EOO, (minInt64 * minInt64).type());
}
...
TEST(Multiplication, BoundsWithNegativeOne) {
    ...
    ASSERT_NOT_EQUALS(monger::EOO, (maxInt64 * minusOneInt64).type());
    ASSERT_NOT_EQUALS(monger::EOO, (minusOneInt64 * maxInt64).type());
    ASSERT_EQUALS(monger::EOO, (minInt64 * minusOneInt64).type());
    ASSERT_EQUALS(monger::EOO, (minusOneInt64 * minInt64).type());
}
