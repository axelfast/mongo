...
(function() {
    ...
    /**
     * Attempt to re-establish and re-authenticate a Monger connection if it was dropped, with
     * multiple retries.
     *
     * Returns upon successful re-connnection. If connection cannot be established after 200
     * retries, throws an exception.
     *
     * @param conn - a Monger connection object or DB object.
     */
        ...
        wait(function() {
            ...
            try {
                ...
                if (jsTest.options().keyFile) {
                    return jsTest.authenticate(db.getMonger());
                }
                return true;
            } catch (e) {
                print(e);
    ...
    getLatestOp = function(server) {
        server.getDB("admin").getMonger().setSlaveOk();
