...
(function() {
    ...
    sleep(10 * 1000);
    assert.eq(0, mock_web.queryStats().registers, "mongerd registered without enabling free_mod");
    assert.eq(
        0, mock_web_sec.queryStats().registers, "mongerd registered without enabling free_mod");
